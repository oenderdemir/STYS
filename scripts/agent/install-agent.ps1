...
$wrapperPath = Join-Path $InstallDir "Start-STYS-Agent.ps1"
$wrapperContent = @"
`$ErrorActionPreference = 'Stop'
Set-Location -LiteralPath '$InstallDir'
Start-Process -FilePath (Join-Path '$InstallDir' 'STYS.Agent.exe') -WorkingDirectory '$InstallDir' -NoNewWindow -Wait
"@
...
$serviceCommand = "`"$powershellExe`" -NoLogo -NoProfile -ExecutionPolicy Bypass -File `"$wrapperPath`""
...
sc.exe create $ServiceName binPath= $serviceCommand start= auto obj= $ServiceAccount DisplayName= $ServiceDisplayName | Out-Null
