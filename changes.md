...
## Rezervasyon Gelir Tahakkuku (Faz 2) — Uygulama (6. Tur)

### Yapılan İşler
Faz 1'de kurulan "ödeme ≠ gelir" ilkesinin ikinci ayağı: check-out sonrası konaklama gelirinin
`SatisBelgesi`/`CariHareket` üzerinden muhasebeye yansıtılması. Önce mimari tasarım ve somut
uygulama planı (kod içermeyen 2 ayrı Artifact dokümanı) onaylandı, sonra uygulandı. Ayrıntı:
`docs/rezervasyon-gelir-tahakkuku-tasarim.md`.

Üç yeni servis: `IRezervasyonCariKartResolver` (Faz 1'deki cari kart mantığının ortaklaştırılması),
`IRezervasyonGelirTahakkukService` (idempotent taslak oluşturma, retroaktif tahsilat kapama, gelir
özeti hesaplama). `RezervasyonSatisBelgesiService` genişletildi: `CariKartId` artık taslağa
set ediliyor, ek hizmet ve restoran (OdayaEkle) satırları da faturaya dahil ediliyor. Check-out
akışı gelir belgesi taslağını best-effort dener — hata check-out'u asla etkilemiyor. Tahsilat
kapatma `CariHareketKapamaService`'e hiçbir değişiklik yapılmadan (yeniden kullanılarak) çalışıyor;
ayrı, manuel bir "Tahsilatları Kapat" aksiyonu olarak kaldı, otomatik zincirlenmiyor.

### Yeni Dosyalar
- backend/Rezervasyonlar/Services/IRezervasyonCariKartResolver.cs, RezervasyonCariKartResolver.cs
- backend/Rezervasyonlar/Services/IRezervasyonGelirTahakkukService.cs, RezervasyonGelirTahakkukService.cs
- backend/Rezervasyonlar/Dto/RezervasyonGelirOzetiDto.cs
- backend/Rezervasyonlar/TahsilatKapamaDurumlari.cs
- backend/Infrastructure/EntityFramework/Migrations/20260714140521_AddRezervasyonSatisBelgesiEntegrasyonu.cs (+Designer)
- tests/STYS.Tests/RezervasyonGelirTahakkukuTests.cs — InMemory, 5 senaryo
- docs/rezervasyon-gelir-tahakkuku-tasarim.md

### Güncellenen Dosyalar
- backend/Rezervasyonlar/Entities/Rezervasyon.cs — SatisBelgesiId eklendi
- backend/Infrastructure/EntityFramework/StysAppDbContext.cs — SatisBelgesiId FK + filtrelenmis unique index
- backend/Rezervasyonlar/Services/RezervasyonOdemeMuhasebeService.cs — cari kart mantigi ortak resolver'a tasindi
- backend/Rezervasyonlar/Services/RezervasyonSatisBelgesiService.cs — CariKartId cozumleme + ek hizmet/restoran satirlari
- backend/Rezervasyonlar/Dtos/RezervasyonSatisBelgesiDtos.cs — CariKartIdOverride eklendi
- backend/Muhasebe/SatisBelgeleri/Dtos/SatisBelgesiTaslakOlusturmaDtos.cs, Services/SatisBelgesiTaslakOlusturmaService.cs — CariKartId alani (genel amacli, additive)
- backend/Rezervasyonlar/Services/RezervasyonService.cs — TamamlaCheckOutAsync'e best-effort gelir belgesi taslak tetikleme + log/bildirim
- backend/Rezervasyonlar/Controllers/RezervasyonController.cs, Program.cs — yeni uc noktalar (gelir-ozeti, gelir-belgesi-olustur, tahsilatlari-kapat) ve DI kayitlari
- frontend/.../rezervasyon-yonetimi.dto.ts, .service.ts — RezervasyonGelirOzetiDto ve yeni servis metotlari
- frontend/.../rezervasyon-odeme-dialog.ts/.html — "Konaklama Geliri" paneli (gelir durumu, tahsilat kapama durumu, aksiyon butonlari)
- tests/STYS.Tests/RezervasyonServiceTests.cs — check-out best-effort testi + FakeRezervasyonGelirTahakkukService
- tests/STYS.Tests/RezervasyonOdemeMuhasebeIntegrationTests.cs — 3 yeni entegrasyon senaryosu (eszamanli olusturma, kapama geri alma, donem kapali), cleanup sirasi FK'ya gore duzeltildi

### Test Sonucu
- Backend + test projesi: `dotnet build` basarili — 0 error
- Frontend: `tsc --noEmit` 0 hata, `ng build` basarili
- Migration: `ZZZ_CheckNoPendingChanges` bos diff verdi (model/snapshot uyumlu)
- InMemory testler (RezervasyonGelirTahakkukuTests.cs): 5/5 basarili
- Entegrasyon testleri (STYS_INTEGRATION_TEST_CONNECTION_STRING ile, stys-mssql/localhost:14333): 11/11 basarili (8 Faz1 + 3 yeni)
- Ortam degiskeni tanimsizken: 11/11 entegrasyon testi Skipped, normal `dotnet test` DB gerektirmiyor
- Genel regresyon: 297 testten 219 basarili, 78 hatali — hepsi Faz 1'de belgelenen pre-existing
  "Aktif kurum bilgisi bulunamadi" imzasiyla (veya ondan bagimsiz, onceden var olan tek
  TenantSecurityTests hatasiyla) eslesiyor; yeni bir regresyon yok
