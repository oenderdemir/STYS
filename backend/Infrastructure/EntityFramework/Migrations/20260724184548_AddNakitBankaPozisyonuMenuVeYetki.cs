...
{
    /// <inheritdoc />
    public partial class AddNakitBankaPozisyonuMenuVeYetki : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(
                ...
                -- Permission rolleri: NakitBankaPozisyonuYonetimi.Menu / .View (salt-okunur ekran - Manage yok)
                ...
                INSERT INTO @RequiredRoles ([Domain], [Name]) VALUES
                    (N'NakitBankaPozisyonuYonetimi', N'Menu'),
                    (N'NakitBankaPozisyonuYonetimi', N'View');
                ...
                DECLARE @MenuRoleId uniqueidentifier = (
                    ...
                    WHERE [Domain] = N'NakitBankaPozisyonuYonetimi' AND [Name] = N'Menu'
                );
                DECLARE @ViewRoleId uniqueidentifier = (
                    SELECT TOP (1) [Id] FROM [TODBase].[Roles]
                    WHERE [Domain] = N'NakitBankaPozisyonuYonetimi' AND [Name] = N'View'
                );
                ...
                -- Nakit ve Banka Pozisyonu menu item
                DECLARE @PozisyonMenuItemId uniqueidentifier;
                SELECT TOP (1) @PozisyonMenuItemId = [Id]
                FROM [TODBase].[MenuItems]
                WHERE [Route] = N'muhasebe/nakit-banka-pozisyonu' AND [IsDeleted] = 0;

                IF @PozisyonMenuItemId IS NULL
                BEGIN
                    SET @PozisyonMenuItemId = NEWID();
                    INSERT INTO [TODBase].[MenuItems] ([Id], [Label], [Icon], [Route], [QueryParams], [ParentId], [MenuOrder], [IsDeleted], [CreatedAt], [UpdatedAt])
                    VALUES (@PozisyonMenuItemId, N'Nakit ve Banka Pozisyonu', N'pi pi-wallet', N'muhasebe/nakit-banka-pozisyonu', NULL, @MuhasebeRootId, 4, 0, @Now, @Now);
                END
                ELSE
                BEGIN
                    UPDATE [TODBase].[MenuItems]
                    SET [Label] = N'Nakit ve Banka Pozisyonu',
                        [Icon] = N'pi pi-wallet',
                        [ParentId] = @MuhasebeRootId,
                        [IsDeleted] = 0,
                        [DeletedAt] = NULL,
                        [UpdatedAt] = @Now
                    WHERE [Id] = @PozisyonMenuItemId;
                END;

                IF NOT EXISTS (
                    SELECT 1 FROM [TODBase].[MenuItemRoles]
                    WHERE [MenuItemId] = @PozisyonMenuItemId AND [RoleId] = @MenuRoleId AND [IsDeleted] = 0
                )
                    INSERT INTO [TODBase].[MenuItemRoles] ([Id], [MenuItemId], [RoleId], [IsDeleted], [CreatedAt], [UpdatedAt])
                    VALUES (NEWID(), @PozisyonMenuItemId, @MenuRoleId, 0, @Now, @Now);
                """);
        }
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(
                ...
                SET [IsDeleted] = 1, [UpdatedAt] = SYSUTCDATETIME()
                WHERE [Domain] = N'NakitBankaPozisyonuYonetimi' AND [IsDeleted] = 0;
                """);
        }
    }
}
