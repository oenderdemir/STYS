...
export class OdaSinifiYonetimi implements OnDestroy {
    ...
    onDialogSave(payload: OdaSinifiDto): void {
        ...
        const save$: Observable<unknown> = this.dialogMode === 'edit' && this.selectedOdaSinifi.id ? this.service.updateOdaSinifi(this.selectedOdaSinifi.id, payload) : this.service.createOdaSinifi(payload);
        ...
        save$
            ...
            .subscribe({
                next: () => {
                    ...
                    this.loadOdaSiniflari(this.pageNumber, this.pageSize);
                    this.messageService.add({ severity: UiSeverity.Success, summary: 'Basarili', detail: this.dialogMode === 'edit' ? 'Oda sinifi guncellendi.' : 'Oda sinifi olusturuldu.' });
                    this.cdr.detectChanges();
                },
                error: (error: unknown) => {
        ...
        this.confirmationService.confirm({
            ...
            accept: () => {
                this.service.deleteOdaSinifi(odaSinifi.id!).subscribe({
                    next: () => {
                        this.loadOdaSiniflari(this.pageNumber, this.pageSize);
                        this.messageService.add({ severity: UiSeverity.Success, summary: 'Basarili', detail: 'Oda sinifi silindi.' });
                        this.cdr.detectChanges();
                    },
                    error: (error: unknown) => {
                        this.messageService.add({ severity: UiSeverity.Error, summary: 'Hata', detail: this.resolveErrorMessage(error) });
