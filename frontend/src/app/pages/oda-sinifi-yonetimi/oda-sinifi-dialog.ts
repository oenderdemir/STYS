...
@Component({
    selector: 'app-oda-sinifi-dialog',
    standalone: true,
...
export class OdaSinifiDialog implements OnChanges {
    @Input() visible = false;
    @Input() mode: CrudDialogMode = 'create';
    @Input() model: OdaSinifiDto = { kod: '', ad: '', aktifMi: true };
    @Input() saving = false;
    @Input() canManage = false;

    @Output() readonly visibleChange = new EventEmitter<boolean>();
    @Output() readonly save = new EventEmitter<OdaSinifiDto>();
    @Output() readonly modeChange = new EventEmitter<CrudDialogMode>();

    workingModel: OdaSinifiDto = { kod: '', ad: '', aktifMi: true };
    ...
    get dialogTitle(): string {
        if (this.mode === 'create') {
            return 'Yeni Oda Sinifi';
        }

        if (this.mode === 'edit') {
            return 'Oda Sinifi Duzenle';
        }

        return 'Oda Sinifi Detay';
    }
    ...
    canSubmit(): boolean {
        return (this.workingModel.kod?.trim() ?? '').length > 0 && (this.workingModel.ad?.trim() ?? '').length > 0;
    }
