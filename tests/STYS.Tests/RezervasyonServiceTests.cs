...
public class RezervasyonServiceTests
{
        ...
        TimeProvider? timeProvider = null)
    {
        ...
        return new RezervasyonService(
            ...
            timeProvider ?? new FixedTimeProvider(new DateTimeOffset(2026, 3, 8, 5, 0, 0, TimeSpan.Zero)));
    }
    ...
    public async Task UzatmaSecenekleri_TekOdaTumAralikUygunDegilAmaGercekSinirdaIkiSegmentKurulabiliyorsa_UzatmaSirasindaOdaDegisimiUretilir()
    {
        ...
        // Oda 101, uzatmanin ILK gecesinde baska rezervasyona bagli - mevcut oda (100) ILK gecede
        // MUSAIT kalmalidir, cunku UzatmaSirasindaOdaDegisimi'nin ILK segmentinde konaklayan CHECKOUT
        // GUNUNDE degil, YALNIZCA uzatma sirasindaki sinirda tasinabilir (bkz.
        // ComputeUzatmaCandidateAssignabilityAsync).
        await SeedDigerRezervasyonuAsync(dbContext, 5023, 5024, odaId: 101, cikis, boundary);
        // Oda 100 (mevcut oda), uzatmanin IKINCI gecesinde baska rezervasyona bagli - konaklayan
        // BURADA (tek sinirda) Oda 101'e tasinir.
        await SeedDigerRezervasyonuAsync(dbContext, 5025, 5026, odaId: 100, boundary, yeniCikis);
    ...
    public async Task UzatmaSecenekleri_AyniSinirdaBirdenFazlaIkiSegmentliAday_IlkAdaydaAramaDurmazVeMevcutTipEslesenTercihEdilir()
    {
        ...
        // uzatmanin ILK gecesinde baska rezervasyonlara bagli - boylece ILK segmentte SADECE mevcut
        // oda (100) musait kalir (UzatmaSirasindaOdaDegisimi'nin ILK segmentinde konaklayan
        // MUTLAKA mevcut odasinda kalmalidir).
        await SeedDigerRezervasyonuAsync(dbContext, 5203, 5204, odaId: 101, cikis, boundary);
        await SeedDigerRezervasyonuAsync(dbContext, 5205, 5206, odaId: 102, cikis, boundary);
        // Mevcut oda (100), uzatmanin IKINCI gecesinde baska rezervasyona bagli - boylece IKINCI
        // gece icin HER IKISI de (greedy=yuksek kapasiteli 102 VE tip-eslesen 101) gecerli birer
        // aday olur.
        await SeedDigerRezervasyonuAsync(dbContext, 5207, 5208, odaId: 100, boundary, yeniCikis);
        ...
        Assert.Contains(result.Secenekler, x => x.SenaryoTipi == RezervasyonUzatmaSenaryoTipleri.UzatmaSirasindaOdaDegisimi
            && x.Segmentler[1].OdaAtamalari.Any(a => a.OdaId == 101));
        ...
        Assert.Equal(100, ilkUzatmaSirasindaSecenek.Segmentler[0].OdaAtamalari.Single().OdaId);
        Assert.Equal(101, ilkUzatmaSirasindaSecenek.Segmentler[1].OdaAtamalari.Single().OdaId);
    }
    ...
    public async Task Uzat_UzatmaSirasindaOdaDegisimiSecilirse_GercekSinirdaIkiYeniSegmentOlusurBoslukVeCakismaOlmaz()
    {
        ...
        await SeedDigerRezervasyonuAsync(dbContext, 6023, 6024, odaId: 101, cikis, boundary);
        await SeedDigerRezervasyonuAsync(dbContext, 6025, 6026, odaId: 100, boundary, yeniCikis);
    ...
    [Fact]
    public async Task ErkenCikis_SegmentSinirinaDenkGelirseSonrakiSegmentleriSiler()
    {
        await using var dbContext = CreateDbContext();
        await SeedReservationFixtureWithTenRoomsAsync(dbContext);
        var giris = new DateTime(2026, 3, 8, 14, 0, 0);
        var sinir = new DateTime(2026, 3, 10, 10, 0, 0);
        var mevcutCikis = new DateTime(2026, 3, 12, 10, 0, 0);
        await SeedUzatmaRezervasyonuAsync(dbContext, 6127, 6128, odaId: 101, giris, mevcutCikis);

        var rezervasyon = await dbContext.Rezervasyonlar.SingleAsync(x => x.Id == 6127);
        rezervasyon.ToplamBazUcret = 3600m;
        rezervasyon.ToplamUcret = 3600m;

        var ilkSegment = await dbContext.RezervasyonSegmentleri.SingleAsync(x => x.Id == 6128);
        ilkSegment.BitisTarihi = sinir;

        var ilkOdaAtama = await dbContext.RezervasyonSegmentOdaAtamalari.SingleAsync(x => x.RezervasyonSegmentId == 6128);
        dbContext.RezervasyonSegmentleri.Add(new RezervasyonSegment
        {
            Id = 6130,
            RezervasyonId = 6127,
            SegmentSirasi = 2,
            BaslangicTarihi = sinir,
            BitisTarihi = mevcutCikis
        });
        dbContext.RezervasyonSegmentOdaAtamalari.Add(new RezervasyonSegmentOdaAtama
        {
            Id = 6131,
            RezervasyonSegmentId = 6130,
            OdaId = ilkOdaAtama.OdaId,
            AyrilanKisiSayisi = ilkOdaAtama.AyrilanKisiSayisi,
            OdaNoSnapshot = ilkOdaAtama.OdaNoSnapshot,
            BinaAdiSnapshot = ilkOdaAtama.BinaAdiSnapshot,
            OdaTipiAdiSnapshot = ilkOdaAtama.OdaTipiAdiSnapshot,
            PaylasimliMiSnapshot = ilkOdaAtama.PaylasimliMiSnapshot,
            KapasiteSnapshot = ilkOdaAtama.KapasiteSnapshot
        });
        dbContext.RezervasyonKonaklayanSegmentAtamalari.Add(new RezervasyonKonaklayanSegmentAtama
        {
            Id = 612703,
            RezervasyonKonaklayanId = 612701,
            RezervasyonSegmentId = 6130,
            OdaId = ilkOdaAtama.OdaId
        });
        await dbContext.SaveChangesAsync();
        ...
        var service = CreateService(dbContext);
        await service.KaydetErkenCikisAsync(6127, new RezervasyonErkenCikisRequestDto { YeniCikisTarihi = sinir });

        var guncel = await dbContext.Rezervasyonlar.SingleAsync(x => x.Id == 6127);
        Assert.Equal(sinir, guncel.CikisTarihi);
        Assert.Equal(1800m, guncel.ToplamUcret);

        var kalanSegment = Assert.Single(await dbContext.RezervasyonSegmentleri.Where(x => x.RezervasyonId == 6127).ToListAsync());
        Assert.Equal(giris, kalanSegment.BaslangicTarihi);
        Assert.Equal(sinir, kalanSegment.BitisTarihi);
        Assert.False(await dbContext.RezervasyonSegmentOdaAtamalari.AnyAsync(x => x.RezervasyonSegmentId == 6130));
        Assert.False(await dbContext.RezervasyonKonaklayanSegmentAtamalari.AnyAsync(x => x.RezervasyonSegmentId == 6130));
    }

    [Fact]
    public async Task ErkenCikis_GecmisTariheCekilirseHataVerir()
    {
        await using var dbContext = CreateDbContext();
        await SeedReservationFixtureWithTenRoomsAsync(dbContext);
        var giris = new DateTime(2026, 3, 8, 14, 0, 0);
        var mevcutCikis = new DateTime(2026, 3, 12, 10, 0, 0);
        var gecmisCikis = new DateTime(2026, 3, 9, 10, 0, 0);
        await SeedUzatmaRezervasyonuAsync(dbContext, 6132, 6133, odaId: 101, giris, mevcutCikis);

        var service = CreateService(dbContext, timeProvider: new FixedTimeProvider(new DateTimeOffset(2026, 3, 10, 9, 0, 0, TimeSpan.Zero)));
        var ex = await Assert.ThrowsAsync<BaseException>(() =>
            service.GetErkenCikisOzetiAsync(6132, new RezervasyonErkenCikisRequestDto { YeniCikisTarihi = gecmisCikis }));

        Assert.Equal(400, ex.ErrorCode);
        Assert.Contains("bugunun tesis cikis saatinden once", ex.Message, StringComparison.OrdinalIgnoreCase);
    }
    ...
    public async Task UzatmaSecenekleri_SadeceCheckOutTamamlanmisKendineOzguTarihFazladanBolmeNoktasiOlusturmaz()
    {
        ...
        await SeedDigerRezervasyonuAsync(dbContext, 6233, 6234, odaId: 101, cikis, boundary);
        await SeedDigerRezervasyonuAsync(dbContext, 6235, 6236, odaId: 100, boundary, yeniCikis);
    ...
    public async Task UzatmaSecenekleri_SadeceIptalKendineOzguTarihFazladanBolmeNoktasiOlusturmaz()
    {
        ...
        await SeedDigerRezervasyonuAsync(dbContext, 6243, 6244, odaId: 101, cikis, boundary);
        await SeedDigerRezervasyonuAsync(dbContext, 6245, 6246, odaId: 100, boundary, yeniCikis);
    ...
    [Fact]
    public async Task ComputeUzatmaGuestAssignmentPlan_GecerliGenelDagilimVarkenErkenAyniOdadaTutmaYuzunden409Uretmez()
    {
        await using var dbContext = CreateDbContext();
        await SeedPaylasimliUzatmaFixtureAsync(dbContext);
        dbContext.Odalar.Add(new Oda { Id = 103, OdaNo = "P-2", BinaId = 10, TesisOdaTipiId = 30, KatNo = 1, AktifMi = true });
        ...
        // Oda 103, uzatma boyunca BASKA bir rezervasyondan KADIN konaklayanla dolu (sabit=Kadin).
        // Oda 101 ise TAMAMEN BOS (hicbir dis rezervasyon yok, cinsiyet acisindan serbest).
        await SeedDigerPaylasimliRezervasyonuAsync(dbContext, 9001, 9002, odaId: 103, baslangic, bitis, KonaklayanCinsiyetleri.Kadin);

        var kendiRezervasyonId = 9010;
        // Kadinin "onceki" odasi TAM OLARAK bos oda (101) - AC GOZLU bir algoritma onu burada
        // TUTMAYA calisir ve erkegi odasiz birakir (103 kadin-sabit) - oysa kadin->103, erkek->101
        // GECERLI bir genel cozumdur.
        var kadinKonaklayan = new RezervasyonKonaklayan { Id = 9011, RezervasyonId = kendiRezervasyonId, SiraNo = 1, AdSoyad = "Kadin Misafir", Cinsiyet = KonaklayanCinsiyetleri.Kadin, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };
        var erkekKonaklayan = new RezervasyonKonaklayan { Id = 9012, RezervasyonId = kendiRezervasyonId, SiraNo = 2, AdSoyad = "Erkek Misafir", Cinsiyet = KonaklayanCinsiyetleri.Erkek, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };
        ...
        var plans = await InvokeComputeUzatmaGuestAssignmentPlanAsync(
            dbContext, kendiRezervasyonId, oncekiOda, new Dictionary<int, int?>(), plan,
            [kadinKonaklayan, erkekKonaklayan], 2);

        Assert.Equal(103, plans.Single(x => x.KonaklayanId == kadinKonaklayan.Id).OdaId);
        Assert.Equal(101, plans.Single(x => x.KonaklayanId == erkekKonaklayan.Id).OdaId);
    }

    [Fact]
    public async Task ComputeUzatmaGuestAssignmentPlan_SiraNoVeOdaIdSirasiTersCevrilsedeAyniSonucAlinir()
    {
        await using var dbContext = CreateDbContext();
        await SeedPaylasimliUzatmaFixtureAsync(dbContext);
        // 100 numarali oda, 101'den KUCUK ID'ye sahip - naif bir "en kucuk ID once" siralamasinin
        // da sonucu yanlislikla tersine cevirmedigini dogrulamak icin BILEREK kullanildi.
        dbContext.Odalar.Add(new Oda { Id = 100, OdaNo = "P-0", BinaId = 10, TesisOdaTipiId = 30, KatNo = 1, AktifMi = true });
        await dbContext.SaveChangesAsync();

        var baslangic = new DateTime(2026, 3, 9, 10, 0, 0);
        var bitis = new DateTime(2026, 3, 10, 10, 0, 0);

        // Oda 100 (kucuk ID), BASKA bir rezervasyondan KADIN konaklayanla dolu (sabit=Kadin).
        await SeedDigerPaylasimliRezervasyonuAsync(dbContext, 9101, 9102, odaId: 100, baslangic, bitis, KonaklayanCinsiyetleri.Kadin);

        var kendiRezervasyonId = 9110;
        // Bu kez ERKEK dusuk SiraNo'ya sahip VE onceki odasi 101 (BOS oda) - siralama (SiraNo VEYA
        // OdaId) sonucu etkilememelidir.
        var erkekKonaklayan = new RezervasyonKonaklayan { Id = 9111, RezervasyonId = kendiRezervasyonId, SiraNo = 1, AdSoyad = "Erkek Misafir", Cinsiyet = KonaklayanCinsiyetleri.Erkek, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };
        var kadinKonaklayan = new RezervasyonKonaklayan { Id = 9112, RezervasyonId = kendiRezervasyonId, SiraNo = 2, AdSoyad = "Kadin Misafir", Cinsiyet = KonaklayanCinsiyetleri.Kadin, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };

        var oncekiOda = new Dictionary<int, int> { [erkekKonaklayan.Id] = 101 };
        var plan = new KonaklamaSenaryoSegmentDto
        {
            BaslangicTarihi = baslangic,
            BitisTarihi = bitis,
            OdaAtamalari =
            [
                new KonaklamaSenaryoOdaAtamaDto { OdaId = 100, OdaNo = "P-0", PaylasimliMi = true, Kapasite = 2, AyrilanKisiSayisi = 1 },
                new KonaklamaSenaryoOdaAtamaDto { OdaId = 101, OdaNo = "P-1", PaylasimliMi = true, Kapasite = 2, AyrilanKisiSayisi = 1 }
            ]
        };
        ...
        var plans = await InvokeComputeUzatmaGuestAssignmentPlanAsync(
            dbContext, kendiRezervasyonId, oncekiOda, new Dictionary<int, int?>(), plan,
            [erkekKonaklayan, kadinKonaklayan], 2);

        Assert.Equal(101, plans.Single(x => x.KonaklayanId == erkekKonaklayan.Id).OdaId);
        Assert.Equal(100, plans.Single(x => x.KonaklayanId == kadinKonaklayan.Id).OdaId);
    }

    [Fact]
    public async Task ComputeUzatmaGuestAssignmentPlan_BirdenFazlaGecerliCozumVarsaEnFazlaKisiOncekiOdasindaTutulur()
    {
        await using var dbContext = CreateDbContext();
        await SeedTwoRoomUzatmaFixtureAsync(dbContext);
        ...
        var kendiRezervasyonId = 9210;
        // Iki konaklayan da hedef dagilimdaki BASKA bir odada da rahatlikla kalabilir (cinsiyet
        // kisitlamasi yok, her iki oda da kapasite 1) - ANCAK "kendi onceki odasinda kalan" cozum
        // (100->100, 101->101) "yer degistiren" cozumden (100->101, 101->100) DAHA FAZLA kisi
        // korur ve TERCIH EDILMELIDIR.
        var konaklayanA = new RezervasyonKonaklayan { Id = 9211, RezervasyonId = kendiRezervasyonId, SiraNo = 1, AdSoyad = "Konaklayan A", KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };
        var konaklayanB = new RezervasyonKonaklayan { Id = 9212, RezervasyonId = kendiRezervasyonId, SiraNo = 2, AdSoyad = "Konaklayan B", KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };

        var oncekiOda = new Dictionary<int, int> { [konaklayanA.Id] = 100, [konaklayanB.Id] = 101 };
        var plan = new KonaklamaSenaryoSegmentDto
        {
            BaslangicTarihi = baslangic,
            BitisTarihi = bitis,
            OdaAtamalari =
            [
                new KonaklamaSenaryoOdaAtamaDto { OdaId = 100, OdaNo = "A-1", PaylasimliMi = false, Kapasite = 1, AyrilanKisiSayisi = 1 },
                new KonaklamaSenaryoOdaAtamaDto { OdaId = 101, OdaNo = "A-2", PaylasimliMi = false, Kapasite = 2, AyrilanKisiSayisi = 1 }
            ]
        };

        var plans = await InvokeComputeUzatmaGuestAssignmentPlanAsync(
            dbContext, kendiRezervasyonId, oncekiOda, new Dictionary<int, int?>(), plan,
            [konaklayanA, konaklayanB], 2);

        Assert.Equal(100, plans.Single(x => x.KonaklayanId == konaklayanA.Id).OdaId);
        Assert.Equal(101, plans.Single(x => x.KonaklayanId == konaklayanB.Id).OdaId);
    }

    [Fact]
    public async Task ComputeUzatmaGuestAssignmentPlan_GercektenGecerliTamEslesmeYoksa409DonerVeKayitBirakmaz()
    {
        await using var dbContext = CreateDbContext();
        await SeedPaylasimliUzatmaFixtureAsync(dbContext);
        ...
        // Oda 101, BASKA bir rezervasyondan KADIN konaklayanla dolu (sabit=Kadin, kalan slot 1).
        await SeedDigerPaylasimliRezervasyonuAsync(dbContext, 9301, 9302, odaId: 101, baslangic, bitis, KonaklayanCinsiyetleri.Kadin);

        var kendiRezervasyonId = 9310;
        // Rezervasyonda 2 kisi (1 kadin + 1 erkek) var ama hedef planda TEK oda (101, kadin-sabit,
        // 1 kisilik) mevcut - erkek icin HICBIR gecerli oda yok, GERCEKTEN cozumsuz.
        var kadinKonaklayan = new RezervasyonKonaklayan { Id = 9311, RezervasyonId = kendiRezervasyonId, SiraNo = 1, AdSoyad = "Kadin Misafir", Cinsiyet = KonaklayanCinsiyetleri.Kadin, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };
        var erkekKonaklayan = new RezervasyonKonaklayan { Id = 9312, RezervasyonId = kendiRezervasyonId, SiraNo = 2, AdSoyad = "Erkek Misafir", Cinsiyet = KonaklayanCinsiyetleri.Erkek, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };
        ...
        var task = (Task)method.Invoke(service, [
            kendiRezervasyonId, new Dictionary<int, int>(), new Dictionary<int, int?>(), plan,
            new List<RezervasyonKonaklayan> { kadinKonaklayan, erkekKonaklayan }, 2, CancellationToken.None
        ])!;

        var ex = await Assert.ThrowsAsync<BaseException>(async () => await task);
        Assert.Equal(409, ex.ErrorCode);

        Assert.Empty(await dbContext.RezervasyonKonaklayanSegmentAtamalari
            .Where(x => x.RezervasyonKonaklayanId == kadinKonaklayan.Id || x.RezervasyonKonaklayanId == erkekKonaklayan.Id)
            .ToListAsync());
    }

    [Fact]
    public async Task ComputeUzatmaGuestAssignmentPlan_GelmediVarkenAktifKadinErkekSabitOdayaAtanmazVeGelmediyeAtamaOlusmaz()
    {
        await using var dbContext = CreateDbContext();
        await SeedPaylasimliUzatmaFixtureAsync(dbContext);
        dbContext.Odalar.Add(new Oda { Id = 103, OdaNo = "P-2", BinaId = 10, TesisOdaTipiId = 30, KatNo = 1, AktifMi = true });
        ...
        // Oda 101, BASKA bir rezervasyondan ERKEK konaklayanla dolu (sabit=Erkek).
        await SeedDigerPaylasimliRezervasyonuAsync(dbContext, 9401, 9402, odaId: 101, baslangic, bitis, KonaklayanCinsiyetleri.Erkek);

        var kendiRezervasyonId = 9410;
        // KisiSayisi=2: bir KADIN Geldi (aktif), bir ERKEK Gelmedi (aktif DEGIL). Eski (hatali) kod
        // aktifKonaklayanlar.Count(1) != KisiSayisi(2) oldugu icin cinsiyet farkindaligini TAMAMEN
        // KAPATIRDI - bu durumda kadin yanlislikla erkek-sabit odaya (101) atanabilirdi.
        var kadinKonaklayan = new RezervasyonKonaklayan { Id = 9411, RezervasyonId = kendiRezervasyonId, SiraNo = 1, AdSoyad = "Kadin Misafir", Cinsiyet = KonaklayanCinsiyetleri.Kadin, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };
        var gelmediKonaklayan = new RezervasyonKonaklayan { Id = 9412, RezervasyonId = kendiRezervasyonId, SiraNo = 2, AdSoyad = "Gelmedi Misafir", Cinsiyet = KonaklayanCinsiyetleri.Erkek, KatilimDurumu = KonaklayanKatilimDurumlari.Gelmedi };
        ...
        var plans = await InvokeComputeUzatmaGuestAssignmentPlanAsync(
            dbContext, kendiRezervasyonId, new Dictionary<int, int>(), new Dictionary<int, int?>(), plan,
            [kadinKonaklayan, gelmediKonaklayan], 2);

        // Yalnizca AKTIF (Geldi) konaklayan icin atama olusur - Gelmedi konaklayan icin YOK.
        var kadinAtama = Assert.Single(plans);
        Assert.Equal(kadinKonaklayan.Id, kadinAtama.KonaklayanId);
        // Kadin, erkek-sabit odaya (101) DEGIL, serbest odaya (103) atanmalidir.
        Assert.Equal(103, kadinAtama.OdaId);
    }

    [Fact]
    public async Task ComputeUzatmaGuestAssignmentPlan_CinsiyetiBilinenKonaklayaninKontroluBaskaKonaklayaninCinsiyetiBosDiyeKapanmaz()
    {
        await using var dbContext = CreateDbContext();
        await SeedPaylasimliUzatmaFixtureAsync(dbContext);
        dbContext.Odalar.Add(new Oda { Id = 103, OdaNo = "P-2", BinaId = 10, TesisOdaTipiId = 30, KatNo = 1, AktifMi = true });
        ...
        // Oda 101, BASKA bir rezervasyondan ERKEK konaklayanla dolu (sabit=Erkek). Oda 102
        // (SeedPaylasimliUzatmaFixtureAsync'ten, tekli/paylasimli OLMAYAN) ve yeni eklenen oda 104
        // (ayni tekli oda tipinden) OZEL/paylasimli-olmayan odalar - cinsiyet kisitlamasi yoktur.
        await SeedDigerPaylasimliRezervasyonuAsync(dbContext, 9501, 9502, odaId: 101, baslangic, bitis, KonaklayanCinsiyetleri.Erkek);
        dbContext.Odalar.Add(new Oda { Id = 104, OdaNo = "T-2", BinaId = 10, TesisOdaTipiId = 31, KatNo = 1, AktifMi = true });
        await dbContext.SaveChangesAsync();

        var kendiRezervasyonId = 9510;
        // Iki AKTIF (Geldi) konaklayan: biri cinsiyeti BILINEN (Kadin), digeri cinsiyeti BILINMEYEN
        // (null). Bilinmeyen konaklayanin varligi, bilinen kadinin erkek-sabit odaya (101)
        // atanmamasi kuralini KAPATMAMALIDIR - VE cinsiyeti bilinmeyen konaklayanin KENDISI de
        // artik (bu round'un muhafazakar politikasi geregi) erkek-sabit odaya (101) ATANAMAZ.
        var kadinKonaklayan = new RezervasyonKonaklayan { Id = 9511, RezervasyonId = kendiRezervasyonId, SiraNo = 1, AdSoyad = "Kadin Misafir", Cinsiyet = KonaklayanCinsiyetleri.Kadin, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };
        var bilinmeyenKonaklayan = new RezervasyonKonaklayan { Id = 9512, RezervasyonId = kendiRezervasyonId, SiraNo = 2, AdSoyad = "Bilinmeyen Misafir", Cinsiyet = null, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };
        ...
        var plan = new KonaklamaSenaryoSegmentDto
        {
            ...
            [
                new KonaklamaSenaryoOdaAtamaDto { OdaId = 101, OdaNo = "P-1", PaylasimliMi = true, Kapasite = 2, AyrilanKisiSayisi = 1 },
                new KonaklamaSenaryoOdaAtamaDto { OdaId = 102, OdaNo = "T-1", PaylasimliMi = false, Kapasite = 1, AyrilanKisiSayisi = 1 },
                new KonaklamaSenaryoOdaAtamaDto { OdaId = 104, OdaNo = "T-2", PaylasimliMi = false, Kapasite = 1, AyrilanKisiSayisi = 1 }
            ]
        };

        var plans = await InvokeComputeUzatmaGuestAssignmentPlanAsync(
            dbContext, kendiRezervasyonId, new Dictionary<int, int>(), new Dictionary<int, int?>(), plan,
            [kadinKonaklayan, bilinmeyenKonaklayan], 2);

        // Ne kadin NE DE bilinmeyen cinsiyetli konaklayan erkek-sabit odaya (101) atanir - ikisi de
        // ozel (paylasimli olmayan) odalara yerlesir.
        Assert.Equal(2, plans.Count);
        Assert.DoesNotContain(plans, x => x.OdaId == 101);
        Assert.Contains(plans, x => x.KonaklayanId == kadinKonaklayan.Id);
        Assert.Contains(plans, x => x.KonaklayanId == bilinmeyenKonaklayan.Id);
    }

    // ─────────────────────────────────────────────────────────────
    // Uzatma kaydet - cinsiyet dogrulamasinin secenek/kaydetme akisinda ORTAKLASTIRILMASI
    // ─────────────────────────────────────────────────────────────

    [Fact]
    public async Task Uzat_KadinGeldiErkekGelmediVeMevcutOdaErkekSabitOldugundaAyniOdadaDevamSunulmazAmaAlternatifPlanKaydedilir()
    {
        await using var dbContext = CreateDbContext();
        await SeedPaylasimliUzatmaFixtureAsync(dbContext);
        ...
        dbContext.RezervasyonSegmentleri.Add(new RezervasyonSegment { Id = 9602, RezervasyonId = rezervasyonId, SegmentSirasi = 1, BaslangicTarihi = girisTarihi, BitisTarihi = cikis });
        dbContext.RezervasyonSegmentOdaAtamalari.Add(new RezervasyonSegmentOdaAtama
        {
            Id = 9603,
            RezervasyonSegmentId = 9602,
            OdaId = 101,
            AyrilanKisiSayisi = 2,
            OdaNoSnapshot = "P-1",
            BinaAdiSnapshot = "Blok",
        ...
        dbContext.RezervasyonKonaklayanlar.AddRange(
            new RezervasyonKonaklayan { Id = 9611, RezervasyonId = rezervasyonId, SiraNo = 1, AdSoyad = "Kadin Misafir", Cinsiyet = KonaklayanCinsiyetleri.Kadin, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi },
            new RezervasyonKonaklayan { Id = 9612, RezervasyonId = rezervasyonId, SiraNo = 2, AdSoyad = "Erkek Misafir", Cinsiyet = KonaklayanCinsiyetleri.Erkek, KatilimDurumu = KonaklayanKatilimDurumlari.Gelmedi });
        dbContext.RezervasyonKonaklayanSegmentAtamalari.AddRange(
            new RezervasyonKonaklayanSegmentAtama { Id = 9621, RezervasyonKonaklayanId = 9611, RezervasyonSegmentId = 9602, OdaId = 101 },
            new RezervasyonKonaklayanSegmentAtama { Id = 9622, RezervasyonKonaklayanId = 9612, RezervasyonSegmentId = 9602, OdaId = 101 });
        await dbContext.SaveChangesAsync();

        // Uzatma boyunca oda 101, BASKA bir rezervasyondan ERKEK konaklayanla dolu (sabit=Erkek) -
        // sayisal kapasite (2-1=1) yeterli olsa da, kadin ORADA KALAMAZ.
        await SeedDigerPaylasimliRezervasyonuAsync(dbContext, 9631, 9632, odaId: 101, cikis, yeniCikis, KonaklayanCinsiyetleri.Erkek);

        var service = CreateService(dbContext);
        var secenekler = await service.GetUzatmaSecenekleriAsync(rezervasyonId, new RezervasyonUzatmaSecenekleriRequestDto { YeniCikisTarihi = yeniCikis });

        // #1: AyniOdadaDevam ARTIK SUNULMAZ (sayisal kapasite yeterli olsa da kisisel cinsiyet
        // uygunlugu saglanamiyor).
        Assert.DoesNotContain(secenekler.Secenekler, x => x.SenaryoTipi == RezervasyonUzatmaSenaryoTipleri.AyniOdadaDevam);

        // #3: Kadina uygun alternatif oda (102, ozel) mevcut oldugu icin CheckoutGunundeOdaDegisimi
        // sunulur ve bu plan BASARIYLA kaydedilebilir.
        var secim = Assert.Single(secenekler.Secenekler, x => x.SenaryoTipi == RezervasyonUzatmaSenaryoTipleri.CheckoutGunundeOdaDegisimi);

        var sonuc = await service.RezervasyonUzatAsync(rezervasyonId, new RezervasyonUzatRequestDto { YeniCikisTarihi = yeniCikis, SenaryoKodu = secim.SenaryoKodu });
        Assert.Equal(yeniCikis, sonuc.YeniCikisTarihi);

        // #7: Gelmedi konaklayan (erkek) icin YENI segmentte atama OLUSTURULMAZ; kadin icin olusur.
        var yeniSegment = await dbContext.RezervasyonSegmentleri.Where(x => x.RezervasyonId == rezervasyonId).OrderByDescending(x => x.SegmentSirasi).FirstAsync();
        var atamalar = await dbContext.RezervasyonKonaklayanSegmentAtamalari.Where(x => x.RezervasyonSegmentId == yeniSegment.Id).ToListAsync();
        var kadinAtama = Assert.Single(atamalar);
        Assert.Equal(9611, kadinAtama.RezervasyonKonaklayanId);
    }

    [Fact]
    public async Task Uzat_AyniOdadaDevamKoduDogrudanGonderilirse409DonerVeSegmentDegismez()
    {
        await using var dbContext = CreateDbContext();
        await SeedPaylasimliUzatmaFixtureAsync(dbContext);
        ...
        dbContext.Rezervasyonlar.Add(new Rezervasyon
        {
            ...
            GirisTarihi = girisTarihi,
            CikisTarihi = cikis,
            MisafirAdiSoyadi = "Gelmedi Test 2",
            MisafirTelefon = "000",
            ToplamBazUcret = 100m,
            ToplamUcret = 100m,
            ParaBirimi = "TRY",
        ...
        dbContext.RezervasyonSegmentleri.Add(new RezervasyonSegment { Id = 9642, RezervasyonId = rezervasyonId, SegmentSirasi = 1, BaslangicTarihi = girisTarihi, BitisTarihi = cikis });
        dbContext.RezervasyonSegmentOdaAtamalari.Add(new RezervasyonSegmentOdaAtama
        {
            Id = 9643,
            RezervasyonSegmentId = 9642,
            OdaId = 101,
            AyrilanKisiSayisi = 2,
            OdaNoSnapshot = "P-1",
            BinaAdiSnapshot = "Blok",
        ...
        dbContext.RezervasyonKonaklayanlar.AddRange(
            new RezervasyonKonaklayan { Id = 9651, RezervasyonId = rezervasyonId, SiraNo = 1, AdSoyad = "Kadin Misafir", Cinsiyet = KonaklayanCinsiyetleri.Kadin, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi },
            new RezervasyonKonaklayan { Id = 9652, RezervasyonId = rezervasyonId, SiraNo = 2, AdSoyad = "Erkek Misafir", Cinsiyet = KonaklayanCinsiyetleri.Erkek, KatilimDurumu = KonaklayanKatilimDurumlari.Gelmedi });
        dbContext.RezervasyonKonaklayanSegmentAtamalari.AddRange(
            new RezervasyonKonaklayanSegmentAtama { Id = 9661, RezervasyonKonaklayanId = 9651, RezervasyonSegmentId = 9642, OdaId = 101 },
            new RezervasyonKonaklayanSegmentAtama { Id = 9662, RezervasyonKonaklayanId = 9652, RezervasyonSegmentId = 9642, OdaId = 101 });
        await dbContext.SaveChangesAsync();

        await SeedDigerPaylasimliRezervasyonuAsync(dbContext, 9671, 9672, odaId: 101, cikis, yeniCikis, KonaklayanCinsiyetleri.Erkek);

        // Istemci, ARTIK sunulmayan AyniOdadaDevam planinin SenaryoKodu'nu (sanki hala gecerliymis
        // gibi) DOGRUDAN uretip kayda gonderir.
        var eskiSenaryoKodu = InvokeCreateUzatmaPlanKodu(
            rezervasyonId, cikis, yeniCikis,
            [new KonaklamaSenaryoSegmentDto { BaslangicTarihi = cikis, BitisTarihi = yeniCikis, OdaAtamalari = [UzatmaOdaAtamasi(101, 2)] }]);

        var service = CreateService(dbContext);
        var segmentOnce = await dbContext.RezervasyonSegmentleri.SingleAsync(x => x.RezervasyonId == rezervasyonId);
        var bitisOnce = segmentOnce.BitisTarihi;

        var ex = await Assert.ThrowsAsync<BaseException>(() =>
            service.RezervasyonUzatAsync(rezervasyonId, new RezervasyonUzatRequestDto { YeniCikisTarihi = yeniCikis, SenaryoKodu = eskiSenaryoKodu }));
        Assert.Equal(409, ex.ErrorCode);

        var segmentSonra = await dbContext.RezervasyonSegmentleri.SingleAsync(x => x.RezervasyonId == rezervasyonId);
        Assert.Equal(bitisOnce, segmentSonra.BitisTarihi);
    }

    [Fact]
    public async Task Uzat_KilitSonrasiOdaCinsiyetiDegisirseKayit409DonerVeDegisiklikBirakmaz()
    {
        await using var dbContext = CreateDbContext();
        await SeedPaylasimliUzatmaFixtureAsync(dbContext);
        ...
        // Oda 101 (paylasimli, cap2) su an TAMAMEN BOS - kadin AyniOdadaDevam ile kalabilir.
        await SeedUzatmaRezervasyonuAsync(dbContext, 9701, 9702, odaId: 101, girisTarihi, cikis, cinsiyet: KonaklayanCinsiyetleri.Kadin);

        var service = CreateService(dbContext);
        var secenekler = await service.GetUzatmaSecenekleriAsync(9701, new RezervasyonUzatmaSecenekleriRequestDto { YeniCikisTarihi = yeniCikis });
        var secim = secenekler.Secenekler.Single(x => x.SenaryoTipi == RezervasyonUzatmaSenaryoTipleri.AyniOdadaDevam);

        // Secenek gosterildikten SONRA (ancak kilit/kayit ONCESI), oda 101 BASKA bir rezervasyondan
        // ERKEK konaklayanla dolar (sabit=Erkek) - kadin ARTIK orada kalamaz.
        await SeedDigerPaylasimliRezervasyonuAsync(dbContext, 9703, 9704, odaId: 101, cikis, yeniCikis, KonaklayanCinsiyetleri.Erkek);

        var segmentOnce = await dbContext.RezervasyonSegmentleri.SingleAsync(x => x.RezervasyonId == 9701);
        var bitisOnce = segmentOnce.BitisTarihi;

        var ex = await Assert.ThrowsAsync<BaseException>(() =>
            service.RezervasyonUzatAsync(9701, new RezervasyonUzatRequestDto { YeniCikisTarihi = yeniCikis, SenaryoKodu = secim.SenaryoKodu }));
        Assert.Equal(409, ex.ErrorCode);

        var segmentSonra = await dbContext.RezervasyonSegmentleri.SingleAsync(x => x.RezervasyonId == 9701);
        Assert.Equal(bitisOnce, segmentSonra.BitisTarihi);
        var reservation = await dbContext.Rezervasyonlar.SingleAsync(x => x.Id == 9701);
        Assert.Equal(cikis, reservation.CikisTarihi);
    }

    // ─────────────────────────────────────────────────────────────
    // Uzatma kaydet - guvenli simetri elemesi (tam durum anahtarli memoization) ve haricî
    // cinsiyeti bilinmeyen doluluk
    // ─────────────────────────────────────────────────────────────

    [Fact]
    public async Task ComputeUzatmaGuestAssignmentPlan_BosPaylasimliVeOzelOdaEsDegerSayilmazGecerliDagilimBulunur()
    {
        await using var dbContext = CreateDbContext();
        await SeedLookupsAsync(dbContext);
        dbContext.Tesisler.Add(new Tesis
        {
            Id = 1, KurumId = 1, Ad = "Simetri Test", IlId = 1, Telefon = "000", Adres = "Adres",
            GirisSaati = new TimeSpan(14, 0, 0), CikisSaati = new TimeSpan(10, 0, 0), AktifMi = true
        });
        dbContext.Binalar.Add(new Bina { Id = 10, TesisId = 1, Ad = "Blok", KatSayisi = 1, AktifMi = true });
        dbContext.OdaTipleri.AddRange(
            new OdaTipi { Id = 60, TesisId = 1, OdaSinifiId = 1, Ad = "Paylasimli", Kapasite = 2, PaylasimliMi = true, AktifMi = true },
            new OdaTipi { Id = 61, TesisId = 1, OdaSinifiId = 1, Ad = "Ozel", Kapasite = 2, PaylasimliMi = false, AktifMi = true });
        // Oda 100 (KUCUK ID) = bos paylasimli; Oda 101 (BUYUK ID) = ozel (paylasimli olmayan).
        dbContext.Odalar.AddRange(
            new Oda { Id = 100, OdaNo = "PAY", BinaId = 10, TesisOdaTipiId = 60, KatNo = 1, AktifMi = true },
            new Oda { Id = 101, OdaNo = "OZEL", BinaId = 10, TesisOdaTipiId = 61, KatNo = 1, AktifMi = true });
        await dbContext.SaveChangesAsync();

        var baslangic = new DateTime(2026, 3, 9, 10, 0, 0);
        var bitis = new DateTime(2026, 3, 10, 10, 0, 0);
        var rezervasyonId = 9801;

        // Birinci konaklayanin cinsiyeti bilinmiyor VE onceki odasi yok; digerleri 1 kadin + 2 erkek.
        var bilinmeyen = new RezervasyonKonaklayan { Id = 9811, RezervasyonId = rezervasyonId, SiraNo = 1, AdSoyad = "Bilinmeyen", Cinsiyet = null, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };
        var kadin = new RezervasyonKonaklayan { Id = 9812, RezervasyonId = rezervasyonId, SiraNo = 2, AdSoyad = "Kadin", Cinsiyet = KonaklayanCinsiyetleri.Kadin, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };
        var erkek1 = new RezervasyonKonaklayan { Id = 9813, RezervasyonId = rezervasyonId, SiraNo = 3, AdSoyad = "Erkek1", Cinsiyet = KonaklayanCinsiyetleri.Erkek, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };
        var erkek2 = new RezervasyonKonaklayan { Id = 9814, RezervasyonId = rezervasyonId, SiraNo = 4, AdSoyad = "Erkek2", Cinsiyet = KonaklayanCinsiyetleri.Erkek, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };
        ...
        var plan = new KonaklamaSenaryoSegmentDto
        {
            ...
            OdaAtamalari =
            [
                new KonaklamaSenaryoOdaAtamaDto { OdaId = 100, OdaNo = "PAY", PaylasimliMi = true, Kapasite = 2, AyrilanKisiSayisi = 2 },
                new KonaklamaSenaryoOdaAtamaDto { OdaId = 101, OdaNo = "OZEL", PaylasimliMi = false, Kapasite = 2, AyrilanKisiSayisi = 2 }
            ]
        };

        var plans = await InvokeComputeUzatmaGuestAssignmentPlanAsync(
            dbContext, rezervasyonId, new Dictionary<int, int>(), new Dictionary<int, int?>(), plan,
            [bilinmeyen, kadin, erkek1, erkek2], 4);

        // Gecerli TEK dagilim: bilinmeyen+kadin -> ozel oda (101); iki erkek -> bos paylasimli (100).
        // Bilinmeyen kisi paylasimli odaya (kucuk ID) yalniz gidip erkeklere yer birakmayacak sekilde
        // YANLIS bir "temsilci" ELENMESINE UGRAMAMALIDIR.
        Assert.Equal(4, plans.Count);
        Assert.Equal(101, plans.Single(x => x.KonaklayanId == bilinmeyen.Id).OdaId);
        Assert.Equal(101, plans.Single(x => x.KonaklayanId == kadin.Id).OdaId);
        Assert.Equal(100, plans.Single(x => x.KonaklayanId == erkek1.Id).OdaId);
        Assert.Equal(100, plans.Single(x => x.KonaklayanId == erkek2.Id).OdaId);
    }

    [Fact]
    public async Task ComputeUzatmaGuestAssignmentPlan_OdaIdleriTersCevrilinceDeGecerliSonucBulunur()
    {
        await using var dbContext = CreateDbContext();
        await SeedLookupsAsync(dbContext);
        dbContext.Tesisler.Add(new Tesis
        {
            Id = 1, KurumId = 1, Ad = "Simetri Ters Test", IlId = 1, Telefon = "000", Adres = "Adres",
            GirisSaati = new TimeSpan(14, 0, 0), CikisSaati = new TimeSpan(10, 0, 0), AktifMi = true
        });
        dbContext.Binalar.Add(new Bina { Id = 10, TesisId = 1, Ad = "Blok", KatSayisi = 1, AktifMi = true });
        dbContext.OdaTipleri.AddRange(
            new OdaTipi { Id = 60, TesisId = 1, OdaSinifiId = 1, Ad = "Ozel", Kapasite = 2, PaylasimliMi = false, AktifMi = true },
            new OdaTipi { Id = 61, TesisId = 1, OdaSinifiId = 1, Ad = "Paylasimli", Kapasite = 2, PaylasimliMi = true, AktifMi = true });
        // Bu kez Oda 100 (KUCUK ID) = ozel; Oda 101 (BUYUK ID) = bos paylasimli - ID'ler TERS.
        dbContext.Odalar.AddRange(
            new Oda { Id = 100, OdaNo = "OZEL", BinaId = 10, TesisOdaTipiId = 60, KatNo = 1, AktifMi = true },
            new Oda { Id = 101, OdaNo = "PAY", BinaId = 10, TesisOdaTipiId = 61, KatNo = 1, AktifMi = true });
        await dbContext.SaveChangesAsync();

        var baslangic = new DateTime(2026, 3, 9, 10, 0, 0);
        var bitis = new DateTime(2026, 3, 10, 10, 0, 0);
        var rezervasyonId = 9821;

        var bilinmeyen = new RezervasyonKonaklayan { Id = 9831, RezervasyonId = rezervasyonId, SiraNo = 1, AdSoyad = "Bilinmeyen", Cinsiyet = null, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };
        var kadin = new RezervasyonKonaklayan { Id = 9832, RezervasyonId = rezervasyonId, SiraNo = 2, AdSoyad = "Kadin", Cinsiyet = KonaklayanCinsiyetleri.Kadin, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };
        var erkek1 = new RezervasyonKonaklayan { Id = 9833, RezervasyonId = rezervasyonId, SiraNo = 3, AdSoyad = "Erkek1", Cinsiyet = KonaklayanCinsiyetleri.Erkek, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };
        var erkek2 = new RezervasyonKonaklayan { Id = 9834, RezervasyonId = rezervasyonId, SiraNo = 4, AdSoyad = "Erkek2", Cinsiyet = KonaklayanCinsiyetleri.Erkek, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };
        ...
        var plan = new KonaklamaSenaryoSegmentDto
        {
            ...
            OdaAtamalari =
            [
                new KonaklamaSenaryoOdaAtamaDto { OdaId = 100, OdaNo = "OZEL", PaylasimliMi = false, Kapasite = 2, AyrilanKisiSayisi = 2 },
                new KonaklamaSenaryoOdaAtamaDto { OdaId = 101, OdaNo = "PAY", PaylasimliMi = true, Kapasite = 2, AyrilanKisiSayisi = 2 }
            ]
        };
        ...
        Assert.Equal(4, plans.Count);
        Assert.Equal(100, plans.Single(x => x.KonaklayanId == bilinmeyen.Id).OdaId);
        Assert.Equal(100, plans.Single(x => x.KonaklayanId == kadin.Id).OdaId);
        Assert.Equal(101, plans.Single(x => x.KonaklayanId == erkek1.Id).OdaId);
        Assert.Equal(101, plans.Single(x => x.KonaklayanId == erkek2.Id).OdaId);
    }

    [Fact]
    public async Task ComputeUzatmaGuestAssignmentPlan_SimetriElemeKalanKonaklayaninOncekiOdasiniElemezEnFazlaKisiKorunur()
    {
        await using var dbContext = CreateDbContext();
        await SeedLookupsAsync(dbContext);
        dbContext.Tesisler.Add(new Tesis
        {
            Id = 1, KurumId = 1, Ad = "Simetri Onceki Oda Test", IlId = 1, Telefon = "000", Adres = "Adres",
            GirisSaati = new TimeSpan(14, 0, 0), CikisSaati = new TimeSpan(10, 0, 0), AktifMi = true
        });
        dbContext.Binalar.Add(new Bina { Id = 10, TesisId = 1, Ad = "Blok", KatSayisi = 1, AktifMi = true });
        dbContext.OdaTipleri.Add(new OdaTipi { Id = 70, TesisId = 1, OdaSinifiId = 1, Ad = "Ozel", Kapasite = 1, PaylasimliMi = false, AktifMi = true });
        // Iki BIREBIR AYNI (kapasite/durum) ozel oda - 200 (kucuk) ve 201 (buyuk).
        dbContext.Odalar.AddRange(
            new Oda { Id = 200, OdaNo = "OZEL-1", BinaId = 10, TesisOdaTipiId = 70, KatNo = 1, AktifMi = true },
            new Oda { Id = 201, OdaNo = "OZEL-2", BinaId = 10, TesisOdaTipiId = 70, KatNo = 1, AktifMi = true });
        await dbContext.SaveChangesAsync();

        var baslangic = new DateTime(2026, 3, 9, 10, 0, 0);
        var bitis = new DateTime(2026, 3, 10, 10, 0, 0);
        var rezervasyonId = 9841;

        // Konaklayan B (SiraNo 1, ONCE islenir), onceki odasi YOK. Konaklayan A (SiraNo 2), onceki
        // odasi 201 (BUYUK ID'li oda) - riskli bir simetri elemesi B'yi yalnizca kucuk ID'li (200)
        // temsilciye zorlayip A'nin GERCEK onceki odasini (201) elinden alabilirdi.
        var konaklayanB = new RezervasyonKonaklayan { Id = 9851, RezervasyonId = rezervasyonId, SiraNo = 1, AdSoyad = "Konaklayan B", KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };
        var konaklayanA = new RezervasyonKonaklayan { Id = 9852, RezervasyonId = rezervasyonId, SiraNo = 2, AdSoyad = "Konaklayan A", KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };

        var oncekiOda = new Dictionary<int, int> { [konaklayanA.Id] = 201 };
        ...
        var plan = new KonaklamaSenaryoSegmentDto
        {
            ...
            OdaAtamalari =
            [
                new KonaklamaSenaryoOdaAtamaDto { OdaId = 200, OdaNo = "OZEL-1", PaylasimliMi = false, Kapasite = 1, AyrilanKisiSayisi = 1 },
                new KonaklamaSenaryoOdaAtamaDto { OdaId = 201, OdaNo = "OZEL-2", PaylasimliMi = false, Kapasite = 1, AyrilanKisiSayisi = 1 }
            ]
        };

        var plans = await InvokeComputeUzatmaGuestAssignmentPlanAsync(
            dbContext, rezervasyonId, oncekiOda, new Dictionary<int, int?>(), plan,
            [konaklayanB, konaklayanA], 2);

        // Konaklayan A, GERCEK onceki odasinda (201) KALMALIDIR - B, A'nin onceki odasini
        // "es deger bir temsilci ugruna" isgal ETMEMELIDIR.
        Assert.Equal(201, plans.Single(x => x.KonaklayanId == konaklayanA.Id).OdaId);
        Assert.Equal(200, plans.Single(x => x.KonaklayanId == konaklayanB.Id).OdaId);
    }

    [Fact]
    public async Task ComputeUzatmaGuestAssignmentPlan_BilinmeyenVeBilinenAyniBosPaylasimliOdadaBirlestirilmez()
    {
        await using var dbContext = CreateDbContext();
        await SeedPaylasimliUzatmaFixtureAsync(dbContext);
        ...
        var baslangic = new DateTime(2026, 3, 9, 10, 0, 0);
        var bitis = new DateTime(2026, 3, 10, 10, 0, 0);
        var rezervasyonId = 9861;

        // Oda 101 (paylasimli, cap2) TAMAMEN BOS - tek hedef oda budur, baska alternatif YOK.
        var bilinmeyen = new RezervasyonKonaklayan { Id = 9871, RezervasyonId = rezervasyonId, SiraNo = 1, AdSoyad = "Bilinmeyen", Cinsiyet = null, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };
        var kadin = new RezervasyonKonaklayan { Id = 9872, RezervasyonId = rezervasyonId, SiraNo = 2, AdSoyad = "Kadin", Cinsiyet = KonaklayanCinsiyetleri.Kadin, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };

        var plan = new KonaklamaSenaryoSegmentDto
        {
            BaslangicTarihi = baslangic,
            BitisTarihi = bitis,
            OdaAtamalari = [new KonaklamaSenaryoOdaAtamaDto { OdaId = 101, OdaNo = "P-1", PaylasimliMi = true, Kapasite = 2, AyrilanKisiSayisi = 2 }]
        };
        ...
        var task = (Task)method.Invoke(service, [
            rezervasyonId, new Dictionary<int, int>(), new Dictionary<int, int?>(), plan,
            new List<RezervasyonKonaklayan> { bilinmeyen, kadin }, 2, CancellationToken.None
        ])!;
    ...
    [Fact]
    public async Task ComputeUzatmaGuestAssignmentPlan_HaricîCinsiyetiBilinmeyenKisiliOdaBosSayilmazVeKimseAtanamaz()
    {
        await using var dbContext = CreateDbContext();
        await SeedPaylasimliUzatmaFixtureAsync(dbContext);
        ...
        // Oda 101'de (paylasimli, cap2) BASKA bir rezervasyondan AKTIF, cinsiyeti BILINMEYEN bir
        // konaklayan kaliyor.
        dbContext.Rezervasyonlar.Add(new Rezervasyon
        {
            Id = 9901, ReferansNo = "DGR-9901", TesisId = 1, KisiSayisi = 1, MisafirTipiId = 1, KonaklamaTipiId = 1,
            GirisTarihi = baslangic, CikisTarihi = bitis, MisafirAdiSoyadi = "Diger Bilinmeyen", MisafirTelefon = "000",
            ToplamBazUcret = 100m, ToplamUcret = 100m, ParaBirimi = "TRY", RezervasyonDurumu = RezervasyonDurumlari.Onayli, AktifMi = true
        });
        dbContext.RezervasyonSegmentleri.Add(new RezervasyonSegment { Id = 9902, RezervasyonId = 9901, SegmentSirasi = 1, BaslangicTarihi = baslangic, BitisTarihi = bitis });
        dbContext.RezervasyonSegmentOdaAtamalari.Add(new RezervasyonSegmentOdaAtama
        {
            Id = 9903, RezervasyonSegmentId = 9902, OdaId = 101, AyrilanKisiSayisi = 1,
            OdaNoSnapshot = "P-1", BinaAdiSnapshot = "Blok", OdaTipiAdiSnapshot = "Paylasimli Cift", PaylasimliMiSnapshot = true, KapasiteSnapshot = 2
        });
        dbContext.RezervasyonKonaklayanlar.Add(new RezervasyonKonaklayan { Id = 9904, RezervasyonId = 9901, SiraNo = 1, AdSoyad = "Diger Bilinmeyen Misafir", Cinsiyet = null, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi });
        dbContext.RezervasyonKonaklayanSegmentAtamalari.Add(new RezervasyonKonaklayanSegmentAtama { Id = 9905, RezervasyonKonaklayanId = 9904, RezervasyonSegmentId = 9902, OdaId = 101 });
        await dbContext.SaveChangesAsync();

        var kendiRezervasyonId = 9910;
        var kadin = new RezervasyonKonaklayan { Id = 9911, RezervasyonId = kendiRezervasyonId, SiraNo = 1, AdSoyad = "Kadin Misafir", Cinsiyet = KonaklayanCinsiyetleri.Kadin, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };
        ...
        // Bilinen (kadin) konaklayan haricî bilinmeyen kisili odaya ATANAMAZ - tek hedef oda budur,
        // dolayisiyla GECERLI hicbir atama yoktur (409).
        var task = (Task)method.Invoke(service, [
            kendiRezervasyonId, new Dictionary<int, int>(), new Dictionary<int, int?>(), plan,
            new List<RezervasyonKonaklayan> { kadin }, 1, CancellationToken.None
        ])!;
        var ex = await Assert.ThrowsAsync<BaseException>(async () => await task);
        Assert.Equal(409, ex.ErrorCode);

        // Cinsiyeti bilinmeyen KENDI konaklayanimiz da (haricî bilinmeyen kisi tek basina kalma
        // hakkini "bosa cikarmaz") ATANAMAZ.
        var bilinmeyenKendi = new RezervasyonKonaklayan { Id = 9912, RezervasyonId = kendiRezervasyonId, SiraNo = 1, AdSoyad = "Bilinmeyen Kendi", Cinsiyet = null, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi };
        var task2 = (Task)method.Invoke(service, [
            kendiRezervasyonId, new Dictionary<int, int>(), new Dictionary<int, int?>(), plan,
            new List<RezervasyonKonaklayan> { bilinmeyenKendi }, 1, CancellationToken.None
        ])!;
        var ex2 = await Assert.ThrowsAsync<BaseException>(async () => await task2);
        Assert.Equal(409, ex2.ErrorCode);
    }

    [Fact]
    public async Task Uzat_KendiTekBilinmeyenKonaklayaniBosPaylasimliOdadaAyniOdadaDevamEdebilir()
    {
        await using var dbContext = CreateDbContext();
        await SeedPaylasimliUzatmaFixtureAsync(dbContext);
        ...
        // Kendi rezervasyonumuzun TEK konaklayani, cinsiyeti bilinmeyen, oda 101'de (paylasimli,
        // cap2) TEK basina - baska hicbir dis dolulugu yok.
        await SeedUzatmaRezervasyonuAsync(dbContext, 9921, 9922, odaId: 101, girisTarihi, cikis, cinsiyet: null);

        var service = CreateService(dbContext);
        var secenekler = await service.GetUzatmaSecenekleriAsync(9921, new RezervasyonUzatmaSecenekleriRequestDto { YeniCikisTarihi = yeniCikis });
        var secim = secenekler.Secenekler.Single(x => x.SenaryoTipi == RezervasyonUzatmaSenaryoTipleri.AyniOdadaDevam);

        var sonuc = await service.RezervasyonUzatAsync(9921, new RezervasyonUzatRequestDto { YeniCikisTarihi = yeniCikis, SenaryoKodu = secim.SenaryoKodu });

        Assert.Equal(yeniCikis, sonuc.YeniCikisTarihi);
        var segment = Assert.Single(await dbContext.RezervasyonSegmentleri.Where(x => x.RezervasyonId == 9921).ToListAsync());
        Assert.Equal(yeniCikis, segment.BitisTarihi);
    }

    [Fact]
    public async Task UzatmaSecenekleri_HaricîBilinmeyenKisiliOdaGecersizPlanOlarakGosterilmez()
    {
        await using var dbContext = CreateDbContext();
        await SeedPaylasimliUzatmaFixtureAsync(dbContext);
        ...
        // Kendi rezervasyonumuz, tek kadin konaklayan, oda 101'de.
        await SeedUzatmaRezervasyonuAsync(dbContext, 9931, 9932, odaId: 101, girisTarihi, cikis, cinsiyet: KonaklayanCinsiyetleri.Kadin);

        // Uzatma boyunca oda 101'e BASKA bir rezervasyondan cinsiyeti BILINMEYEN aktif bir
        // konaklayan yerlesir.
        dbContext.Rezervasyonlar.Add(new Rezervasyon
        {
            Id = 9941, ReferansNo = "DGR-9941", TesisId = 1, KisiSayisi = 1, MisafirTipiId = 1, KonaklamaTipiId = 1,
            GirisTarihi = cikis, CikisTarihi = yeniCikis, MisafirAdiSoyadi = "Diger Bilinmeyen 2", MisafirTelefon = "000",
            ToplamBazUcret = 100m, ToplamUcret = 100m, ParaBirimi = "TRY", RezervasyonDurumu = RezervasyonDurumlari.Onayli, AktifMi = true
        });
        dbContext.RezervasyonSegmentleri.Add(new RezervasyonSegment { Id = 9942, RezervasyonId = 9941, SegmentSirasi = 1, BaslangicTarihi = cikis, BitisTarihi = yeniCikis });
        dbContext.RezervasyonSegmentOdaAtamalari.Add(new RezervasyonSegmentOdaAtama
        {
            Id = 9943, RezervasyonSegmentId = 9942, OdaId = 101, AyrilanKisiSayisi = 1,
            OdaNoSnapshot = "P-1", BinaAdiSnapshot = "Blok", OdaTipiAdiSnapshot = "Paylasimli Cift", PaylasimliMiSnapshot = true, KapasiteSnapshot = 2
        });
        dbContext.RezervasyonKonaklayanlar.Add(new RezervasyonKonaklayan { Id = 9944, RezervasyonId = 9941, SiraNo = 1, AdSoyad = "Diger Bilinmeyen Misafir 2", Cinsiyet = null, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi });
        dbContext.RezervasyonKonaklayanSegmentAtamalari.Add(new RezervasyonKonaklayanSegmentAtama { Id = 9945, RezervasyonKonaklayanId = 9944, RezervasyonSegmentId = 9942, OdaId = 101 });
        await dbContext.SaveChangesAsync();

        var service = CreateService(dbContext);
        var secenekler = await service.GetUzatmaSecenekleriAsync(9931, new RezervasyonUzatmaSecenekleriRequestDto { YeniCikisTarihi = yeniCikis });

        // AyniOdadaDevam ARTIK SUNULMAMALIDIR (kadin, haricî cinsiyeti bilinmeyen kisili odada
        // KALAMAZ) - ve sunulan HERHANGI bir alternatif plan (varsa) GERCEKTEN kaydedilebilir olmalidir.
        Assert.DoesNotContain(secenekler.Secenekler, x => x.SenaryoTipi == RezervasyonUzatmaSenaryoTipleri.AyniOdadaDevam);
        foreach (var secim in secenekler.Secenekler)
        {
            Assert.DoesNotContain(secim.Segmentler.SelectMany(s => s.OdaAtamalari), a => a.OdaId == 101);
        }
    }

    // ─────────────────────────────────────────────────────────────
    // Uzatma seceneklerinde tek segmentten iki segmente GECIS KARARI - kisi bazli dogrulamaya
    // dayanmali, yalnizca oda duzeyindeki musaitlige guvenilmemeli
    // ─────────────────────────────────────────────────────────────

    /// <summary>
    /// Oda 101 (paylasimli, cap2) tum uzatma araliginda SAYISAL kapasiteye sahip (bir yatagi BASKA
    /// rezervasyondan cinsiyeti BILINMEYEN aktif bir kisi kullaniyor) - bu yuzden oda-duzeyindeki
    /// musaitlik hesaplamasi (ve onun kullandigi, cinsiyeti bilinmeyenleri FILTRELEYEN
    /// GetSharedRoomGuestOccupanciesAsync/GetDistinctSharedRoomGenders) 101'i GECERLI bir tam-aralik
    /// tek segment adayi olarak dogurur; ancak KISI BAZLI dogrulama (GetSharedRoomGuestOccupanciesIncludingUnknownGenderAsync
    /// ile) bu odayi kendi rezervasyonumuz icin TAMAMEN KAPALI bulur ve reddeder. Mevcut/orijinal oda
    /// (100) yalnizca UZATMANIN ILK yarisinda, oda 103 (paylasimli) ise yalnizca IKINCI yarisinda
    /// musaittir - UzatmaSirasindaOdaDegisimi'nin ILK segmentinde konaklayan mevcut odasinda (100)
    /// KALMALIDIR (checkout gununde degil, yalnizca uzatma sirasindaki sinirda tasinabilir) - gecerli
    /// tek plan 100 -> 103 iki-segmentli gecistir (TEK gercek hareket).
    /// </summary>
    private static async Task<(int RezervasyonId, DateTime Cikis, DateTime Boundary, DateTime YeniCikis)> SeedIkiSegmentFallbackFixtureAsync(StysAppDbContext dbContext)
    {
        await SeedLookupsAsync(dbContext);
        dbContext.Tesisler.Add(new Tesis
        {
            Id = 1, KurumId = 1, Ad = "Iki Segment Fallback Test", IlId = 1, Telefon = "000", Adres = "Adres",
            GirisSaati = new TimeSpan(14, 0, 0), CikisSaati = new TimeSpan(10, 0, 0), AktifMi = true
        });
        dbContext.Binalar.Add(new Bina { Id = 10, TesisId = 1, Ad = "Blok", KatSayisi = 1, AktifMi = true });
        dbContext.OdaTipleri.AddRange(
            new OdaTipi { Id = 80, TesisId = 1, OdaSinifiId = 1, Ad = "Orijinal", Kapasite = 1, PaylasimliMi = false, AktifMi = true },
            new OdaTipi { Id = 81, TesisId = 1, OdaSinifiId = 1, Ad = "Paylasimli", Kapasite = 2, PaylasimliMi = true, AktifMi = true });
        dbContext.Odalar.AddRange(
            new Oda { Id = 100, OdaNo = "ORIJINAL", BinaId = 10, TesisOdaTipiId = 80, KatNo = 1, AktifMi = true },
            new Oda { Id = 101, OdaNo = "PAY-1", BinaId = 10, TesisOdaTipiId = 81, KatNo = 1, AktifMi = true },
            new Oda { Id = 103, OdaNo = "PAY-2", BinaId = 10, TesisOdaTipiId = 81, KatNo = 1, AktifMi = true });
        dbContext.OdaFiyatlari.AddRange(
            new OdaFiyat { Id = 9950, TesisOdaTipiId = 80, KonaklamaTipiId = 1, MisafirTipiId = 1, KisiSayisi = 1, Fiyat = 400m, ParaBirimi = "TRY", BaslangicTarihi = new DateTime(2026, 3, 1), BitisTarihi = new DateTime(2026, 3, 31), AktifMi = true },
            new OdaFiyat { Id = 9951, TesisOdaTipiId = 81, KonaklamaTipiId = 1, MisafirTipiId = 1, KisiSayisi = 1, Fiyat = 500m, ParaBirimi = "TRY", BaslangicTarihi = new DateTime(2026, 3, 1), BitisTarihi = new DateTime(2026, 3, 31), AktifMi = true });
        await dbContext.SaveChangesAsync();
        ...
        var boundary = new DateTime(2026, 3, 10, 10, 0, 0);
        var yeniCikis = new DateTime(2026, 3, 11, 10, 0, 0);
        var rezervasyonId = 9961;

        await SeedUzatmaRezervasyonuAsync(dbContext, rezervasyonId, 9962, odaId: 100, girisTarihi, cikis, cinsiyet: KonaklayanCinsiyetleri.Kadin);

        // Orijinal oda (100), uzatmanin IKINCI yarisinda (boundary->yeniCikis) baska bir rezervasyona
        // bagli - yani AyniOdadaDevam/tam-aralik CheckoutGunundeOdaDegisimi devre disidir, ama ILK
        // yarida (cikis->boundary) konaklayan MEVCUT odasinda KALABILIR.
        await SeedDigerRezervasyonuAsync(dbContext, 9963, 9964, odaId: 100, boundary, yeniCikis);

        // Oda 101'de, uzatma boyunca (TUM aralikta) BASKA bir rezervasyondan cinsiyeti BILINMEYEN
        // aktif bir konaklayan kaliyor - sayisal kapasite (2-1=1) yeterli GORUNUR ama kisi bazli
        // dogrulama bu odayi TAMAMEN KAPALI bulmalidir.
        dbContext.Rezervasyonlar.Add(new Rezervasyon
        {
            Id = 9965, ReferansNo = "DGR-9965", TesisId = 1, KisiSayisi = 1, MisafirTipiId = 1, KonaklamaTipiId = 1,
            GirisTarihi = cikis, CikisTarihi = yeniCikis, MisafirAdiSoyadi = "Diger Bilinmeyen", MisafirTelefon = "000",
            ToplamBazUcret = 100m, ToplamUcret = 100m, ParaBirimi = "TRY", RezervasyonDurumu = RezervasyonDurumlari.Onayli, AktifMi = true
        });
        dbContext.RezervasyonSegmentleri.Add(new RezervasyonSegment { Id = 9966, RezervasyonId = 9965, SegmentSirasi = 1, BaslangicTarihi = cikis, BitisTarihi = yeniCikis });
        dbContext.RezervasyonSegmentOdaAtamalari.Add(new RezervasyonSegmentOdaAtama
        {
            Id = 9967, RezervasyonSegmentId = 9966, OdaId = 101, AyrilanKisiSayisi = 1,
            OdaNoSnapshot = "PAY-1", BinaAdiSnapshot = "Blok", OdaTipiAdiSnapshot = "Paylasimli", PaylasimliMiSnapshot = true, KapasiteSnapshot = 2
        });
        dbContext.RezervasyonKonaklayanlar.Add(new RezervasyonKonaklayan { Id = 9968, RezervasyonId = 9965, SiraNo = 1, AdSoyad = "Diger Bilinmeyen Misafir", Cinsiyet = null, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi });
        dbContext.RezervasyonKonaklayanSegmentAtamalari.Add(new RezervasyonKonaklayanSegmentAtama { Id = 9969, RezervasyonKonaklayanId = 9968, RezervasyonSegmentId = 9966, OdaId = 101 });

        // Oda 103 (paylasimli, cap2), yalnizca uzatmanin ILK yarisinda (cikis->boundary) baska bir
        // rezervasyona bagli - yani yalnizca IKINCI yarida (boundary->yeniCikis) musaittir.
        await SeedDigerRezervasyonuAsync(dbContext, 9973, 9974, odaId: 103, cikis, boundary);

        await dbContext.SaveChangesAsync();

        return (rezervasyonId, cikis, boundary, yeniCikis);
    }

    [Fact]
    public async Task UzatmaSecenekleri_OdaDuzeyindeMusaitAmaKisiBazliGecersizTekSegmentVarken_IkiSegmentliPlanKacirilmaz()
    {
        await using var dbContext = CreateDbContext();
        var (rezervasyonId, cikis, boundary, yeniCikis) = await SeedIkiSegmentFallbackFixtureAsync(dbContext);

        var service = CreateService(dbContext);
        var secenekler = await service.GetUzatmaSecenekleriAsync(rezervasyonId, new RezervasyonUzatmaSecenekleriRequestDto { YeniCikisTarihi = yeniCikis });

        // MusaitlikYok DONMEMELI - gecerli iki segmentli plan (100 -> 103, TEK gercek hareket) BULUNMALIDIR.
        Assert.Equal(RezervasyonUzatmaSonucKodlari.SecenekBulundu, secenekler.SonucKodu);
        var secim = Assert.Single(secenekler.Secenekler, x => x.SenaryoTipi == RezervasyonUzatmaSenaryoTipleri.UzatmaSirasindaOdaDegisimi);
        ...
        Assert.Equal(boundary, secim.Segmentler[1].BaslangicTarihi);
        Assert.Equal(100, secim.Segmentler[0].OdaAtamalari.Single().OdaId);
        Assert.Equal(103, secim.Segmentler[1].OdaAtamalari.Single().OdaId);
        Assert.Equal(1, secim.OdaDegisimSayisi);

        // Gecersiz oda 101, HICBIR sunulan planda YER ALMAMALIDIR.
        foreach (var oneri in secenekler.Secenekler)
        {
            Assert.DoesNotContain(oneri.Segmentler.SelectMany(s => s.OdaAtamalari), a => a.OdaId == 101);
        }

        // Donen iki segmentli secenek RezervasyonUzatAsync ile BASARIYLA kaydedilebilmelidir.
        var sonuc = await service.RezervasyonUzatAsync(rezervasyonId, new RezervasyonUzatRequestDto { YeniCikisTarihi = yeniCikis, SenaryoKodu = secim.SenaryoKodu });
        Assert.Equal(yeniCikis, sonuc.YeniCikisTarihi);

        var segmentler = await dbContext.RezervasyonSegmentleri.Where(x => x.RezervasyonId == rezervasyonId).OrderBy(x => x.SegmentSirasi).ToListAsync();
        Assert.Equal(3, segmentler.Count); // orijinal + iki yeni segment
        var yeniSegment1 = segmentler[1];
        var yeniSegment2 = segmentler[2];

        var atama1 = await dbContext.RezervasyonSegmentOdaAtamalari.Where(x => x.RezervasyonSegmentId == yeniSegment1.Id).ToListAsync();
        var atama2 = await dbContext.RezervasyonSegmentOdaAtamalari.Where(x => x.RezervasyonSegmentId == yeniSegment2.Id).ToListAsync();
        Assert.Equal(100, Assert.Single(atama1).OdaId);
        Assert.Equal(103, Assert.Single(atama2).OdaId);

        var konaklayanAtama1 = await dbContext.RezervasyonKonaklayanSegmentAtamalari.Where(x => x.RezervasyonSegmentId == yeniSegment1.Id).ToListAsync();
        var konaklayanAtama2 = await dbContext.RezervasyonKonaklayanSegmentAtamalari.Where(x => x.RezervasyonSegmentId == yeniSegment2.Id).ToListAsync();
        Assert.Equal(100, Assert.Single(konaklayanAtama1).OdaId);
        Assert.Equal(103, Assert.Single(konaklayanAtama2).OdaId);
    }

    [Fact]
    public async Task UzatmaSecenekleri_KisiBazliGecerliTamAralikSecenekVarken_GereksizIkiSegmentliUretilmez()
    {
        await using var dbContext = CreateDbContext();
        await SeedPaylasimliUzatmaFixtureAsync(dbContext);
        ...
        // Oda 101 (paylasimli, cap2) TAMAMEN BOS - kadin icin GECERLI bir tam-aralik AyniOdadaDevam
        // secenegi mevcuttur; gereksiz bir iki segmentli arama YAPILMAMALIDIR.
        await SeedUzatmaRezervasyonuAsync(dbContext, 9981, 9982, odaId: 101, girisTarihi, cikis, cinsiyet: KonaklayanCinsiyetleri.Kadin);

        var service = CreateService(dbContext);
        var secenekler = await service.GetUzatmaSecenekleriAsync(9981, new RezervasyonUzatmaSecenekleriRequestDto { YeniCikisTarihi = yeniCikis });

        Assert.Equal(RezervasyonUzatmaSonucKodlari.SecenekBulundu, secenekler.SonucKodu);
        Assert.Contains(secenekler.Secenekler, x => x.SenaryoTipi == RezervasyonUzatmaSenaryoTipleri.AyniOdadaDevam);
        Assert.DoesNotContain(secenekler.Secenekler, x => x.SenaryoTipi == RezervasyonUzatmaSenaryoTipleri.UzatmaSirasindaOdaDegisimi);
    }

    [Fact]
    public async Task Uzat_IkiSegmentliSecenekAlindiktanSonraHaricîBilinmeyenKisiEklenirseEskiSenaryoKoduIle409DonerVeDegisiklikBirakmaz()
    {
        await using var dbContext = CreateDbContext();
        var (rezervasyonId, cikis, boundary, yeniCikis) = await SeedIkiSegmentFallbackFixtureAsync(dbContext);
        ...
        // Secenek ALINDIKTAN SONRA, oda 103'e (ikinci segmentin PAYLASIMLI hedef odasi) BASKA bir
        // rezervasyondan cinsiyeti BILINMEYEN aktif bir konaklayan yerlesir - plan ARTIK gecerli
        // degildir. Kasitli olarak PAYLASIMLI (kapasitesi 1 olan OZEL degil) bir oda kullanilir -
        // boylece reddin GERCEKTEN cinsiyet-bilinmeyen-engeli (ExternalUnknownGenderBlockedSentinel)
        // tarafindan mi, yoksa yalnizca kapasite dolmasi yuzunden mi tetiklendigi belirsiz kalmaz:
        // oda 103 (cap2) hala 1 bos yatak icerir, dolayisiyla SAYISAL kapasite tek basina bu
        // konaklayani REDDETMEZ - yalnizca kisi bazli cinsiyet dogrulamasi reddeder.
        dbContext.Rezervasyonlar.Add(new Rezervasyon
        {
            Id = 9975, ReferansNo = "DGR-9975", TesisId = 1, KisiSayisi = 1, MisafirTipiId = 1, KonaklamaTipiId = 1,
            GirisTarihi = boundary, CikisTarihi = yeniCikis, MisafirAdiSoyadi = "Sonradan Bilinmeyen", MisafirTelefon = "000",
            ToplamBazUcret = 100m, ToplamUcret = 100m, ParaBirimi = "TRY", RezervasyonDurumu = RezervasyonDurumlari.Onayli, AktifMi = true
        });
        dbContext.RezervasyonSegmentleri.Add(new RezervasyonSegment { Id = 9976, RezervasyonId = 9975, SegmentSirasi = 1, BaslangicTarihi = boundary, BitisTarihi = yeniCikis });
        dbContext.RezervasyonSegmentOdaAtamalari.Add(new RezervasyonSegmentOdaAtama
        {
            Id = 9977, RezervasyonSegmentId = 9976, OdaId = 103, AyrilanKisiSayisi = 1,
            OdaNoSnapshot = "PAY-2", BinaAdiSnapshot = "Blok", OdaTipiAdiSnapshot = "Paylasimli", PaylasimliMiSnapshot = true, KapasiteSnapshot = 2
        });
        dbContext.RezervasyonKonaklayanlar.Add(new RezervasyonKonaklayan { Id = 9978, RezervasyonId = 9975, SiraNo = 1, AdSoyad = "Sonradan Bilinmeyen Misafir", Cinsiyet = null, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi });
        dbContext.RezervasyonKonaklayanSegmentAtamalari.Add(new RezervasyonKonaklayanSegmentAtama { Id = 9979, RezervasyonKonaklayanId = 9978, RezervasyonSegmentId = 9976, OdaId = 103 });
        await dbContext.SaveChangesAsync();

        var segmentSayisiOnce = await dbContext.RezervasyonSegmentleri.CountAsync(x => x.RezervasyonId == rezervasyonId);
        var odaAtamaSayisiOnce = await dbContext.RezervasyonSegmentOdaAtamalari.CountAsync(x => x.RezervasyonSegment!.RezervasyonId == rezervasyonId);
        var konaklayanAtamaSayisiOnce = await dbContext.RezervasyonKonaklayanSegmentAtamalari.CountAsync(x => x.RezervasyonKonaklayan!.RezervasyonId == rezervasyonId);
        var hakSayisiOnce = await dbContext.RezervasyonKonaklamaHaklari.CountAsync(x => x.RezervasyonId == rezervasyonId);
        var reservationOnce = await dbContext.Rezervasyonlar.SingleAsync(x => x.Id == rezervasyonId);
        var cikisOnce = reservationOnce.CikisTarihi;
        var bazUcretOnce = reservationOnce.ToplamBazUcret;
        var ucretOnce = reservationOnce.ToplamUcret;
        var gecmisSayisiOnce = await dbContext.RezervasyonDegisiklikGecmisleri.CountAsync(x => x.RezervasyonId == rezervasyonId && x.IslemTipi == RezervasyonGecmisIslemTipleri.Uzatildi);

        var ex = await Assert.ThrowsAsync<BaseException>(() =>
            service.RezervasyonUzatAsync(rezervasyonId, new RezervasyonUzatRequestDto { YeniCikisTarihi = yeniCikis, SenaryoKodu = secim.SenaryoKodu }));
        Assert.Equal(409, ex.ErrorCode);

        var reservationSonra = await dbContext.Rezervasyonlar.SingleAsync(x => x.Id == rezervasyonId);
        Assert.Equal(cikisOnce, reservationSonra.CikisTarihi);
        Assert.Equal(bazUcretOnce, reservationSonra.ToplamBazUcret);
        Assert.Equal(ucretOnce, reservationSonra.ToplamUcret);
        Assert.Equal(segmentSayisiOnce, await dbContext.RezervasyonSegmentleri.CountAsync(x => x.RezervasyonId == rezervasyonId));
        Assert.Equal(odaAtamaSayisiOnce, await dbContext.RezervasyonSegmentOdaAtamalari.CountAsync(x => x.RezervasyonSegment!.RezervasyonId == rezervasyonId));
        Assert.Equal(konaklayanAtamaSayisiOnce, await dbContext.RezervasyonKonaklayanSegmentAtamalari.CountAsync(x => x.RezervasyonKonaklayan!.RezervasyonId == rezervasyonId));
        Assert.Equal(hakSayisiOnce, await dbContext.RezervasyonKonaklamaHaklari.CountAsync(x => x.RezervasyonId == rezervasyonId));
        Assert.Equal(gecmisSayisiOnce, await dbContext.RezervasyonDegisiklikGecmisleri.CountAsync(x => x.RezervasyonId == rezervasyonId && x.IslemTipi == RezervasyonGecmisIslemTipleri.Uzatildi));
    }

    [Fact]
    public async Task ComputeUzatmaCandidateAssignabilityAsync_TekKonaklayanIkiKezOdaDegistirirse_IkiGercekHareketTespitEdilir()
    {
        await using var dbContext = CreateDbContext();
        await SeedThreeRoomUzatmaFixtureAsync(dbContext);
        ...
        var boundary = new DateTime(2026, 3, 10, 10, 0, 0);
        var yeniCikis = new DateTime(2026, 3, 11, 10, 0, 0);
        var rezervasyonId = await SeedUzatmaRezervasyonuAsync(dbContext, 8101, 8102, odaId: 100, girisTarihi, cikis);
        var konaklayanId = rezervasyonId * 100 + 1;

        var tumKonaklayanlar = await dbContext.RezervasyonKonaklayanlar.Where(x => x.RezervasyonId == rezervasyonId).ToListAsync();
        var oncekiOda = new Dictionary<int, int> { [konaklayanId] = 100 };
        var oncekiYatak = new Dictionary<int, int?> { [konaklayanId] = null };

        // Konaklayan checkout gununde 100 -> 101'e, uzatma sirasindaki sinirda ise 101 -> 102'ye
        // tasinir - TOPLAMDA IKI gercek (KonaklayanId bazli) oda gecisi.
        var segmentler = new List<KonaklamaSenaryoSegmentDto>
        {
            new() { BaslangicTarihi = cikis, BitisTarihi = boundary, OdaAtamalari = [UzatmaOdaAtamasi(101, 1)] },
            new() { BaslangicTarihi = boundary, BitisTarihi = yeniCikis, OdaAtamalari = [UzatmaOdaAtamasi(102, 1)] }
        };

        var sonuc = await InvokeComputeUzatmaCandidateAssignabilityAsync(
            dbContext, rezervasyonId, oncekiOda, oncekiYatak, segmentler, tumKonaklayanlar, requireNoMovesForFirstSegment: false);

        Assert.Equal(2, sonuc.ToplamGercekHareket);
    }

    [Fact]
    public async Task ComputeUzatmaCandidateAssignabilityAsync_EnFazlaBirDegisiklikPolitikasindaIkiGercekHareketliPlanGecersizSayilir()
    {
        await using var dbContext = CreateDbContext();
        await SeedThreeRoomUzatmaFixtureAsync(dbContext);
        ...
        var boundary = new DateTime(2026, 3, 10, 10, 0, 0);
        var yeniCikis = new DateTime(2026, 3, 11, 10, 0, 0);
        var rezervasyonId = await SeedUzatmaRezervasyonuAsync(dbContext, 8111, 8112, odaId: 100, girisTarihi, cikis);
        var konaklayanId = rezervasyonId * 100 + 1;
        ...
        // Ayni "100 -> 101 -> 102" iki-hareketli plan; bu kez UzatmaSirasindaOdaDegisimi'nin
        // gercek kullanim sekliyle (ILK segmentte hareket YASAK) dogrulanir - "en fazla bir
        // degisiklik" politikasinda bu plan GECERSIZ sayilmalidir.
        var segmentler = new List<KonaklamaSenaryoSegmentDto>
        {
            new() { BaslangicTarihi = cikis, BitisTarihi = boundary, OdaAtamalari = [UzatmaOdaAtamasi(101, 1)] },
            new() { BaslangicTarihi = boundary, BitisTarihi = yeniCikis, OdaAtamalari = [UzatmaOdaAtamasi(102, 1)] }
        ...
        var sonuc = await InvokeComputeUzatmaCandidateAssignabilityAsync(
            dbContext, rezervasyonId, oncekiOda, oncekiYatak, segmentler, tumKonaklayanlar, requireNoMovesForFirstSegment: true);

        Assert.False(sonuc.GecerliMi);
    }

    [Fact]
    public async Task ComputeUzatmaCandidateAssignabilityAsync_TopluOdaSayisiDegisiklikGostermeseDeIkiKonaklayanGercektenYerDegistirirseReddedilir()
    {
        await using var dbContext = CreateDbContext();
        await SeedLookupsAsync(dbContext);
        dbContext.Tesisler.Add(new Tesis
        {
            Id = 1, KurumId = 1, Ad = "Takas Test", IlId = 1, Telefon = "000", Adres = "Adres",
            GirisSaati = new TimeSpan(14, 0, 0), CikisSaati = new TimeSpan(10, 0, 0), AktifMi = true
        });
        dbContext.Binalar.Add(new Bina { Id = 10, TesisId = 1, Ad = "Blok", KatSayisi = 1, AktifMi = true });
        dbContext.OdaTipleri.Add(new OdaTipi { Id = 60, TesisId = 1, OdaSinifiId = 1, Ad = "Paylasimli Cift", Kapasite = 2, PaylasimliMi = true, AktifMi = true });
        dbContext.Odalar.AddRange(
            new Oda { Id = 601, OdaNo = "PAY-A", BinaId = 10, TesisOdaTipiId = 60, KatNo = 1, AktifMi = true },
            new Oda { Id = 602, OdaNo = "PAY-B", BinaId = 10, TesisOdaTipiId = 60, KatNo = 1, AktifMi = true });
        dbContext.OdaFiyatlari.Add(new OdaFiyat { Id = 9500, TesisOdaTipiId = 60, KonaklamaTipiId = 1, MisafirTipiId = 1, KisiSayisi = 1, Fiyat = 400m, ParaBirimi = "TRY", BaslangicTarihi = new DateTime(2026, 3, 1), BitisTarihi = new DateTime(2026, 3, 31), AktifMi = true });
        await dbContext.SaveChangesAsync();
        ...
        dbContext.Rezervasyonlar.Add(new Rezervasyon
        {
            Id = 8201, ReferansNo = "TAKAS-8201", TesisId = 1, KisiSayisi = 2, MisafirTipiId = 1, KonaklamaTipiId = 1,
            GirisTarihi = girisTarihi, CikisTarihi = cikis, MisafirAdiSoyadi = "Takas Test", MisafirTelefon = "000",
            ToplamBazUcret = 100m, ToplamUcret = 100m, ParaBirimi = "TRY", RezervasyonDurumu = RezervasyonDurumlari.CheckInTamamlandi, AktifMi = true
        });
        dbContext.RezervasyonSegmentleri.Add(new RezervasyonSegment { Id = 8202, RezervasyonId = 8201, SegmentSirasi = 1, BaslangicTarihi = girisTarihi, BitisTarihi = cikis });
        dbContext.RezervasyonSegmentOdaAtamalari.AddRange(
            new RezervasyonSegmentOdaAtama { Id = 8203, RezervasyonSegmentId = 8202, OdaId = 601, AyrilanKisiSayisi = 1, OdaNoSnapshot = "PAY-A", BinaAdiSnapshot = "Blok", OdaTipiAdiSnapshot = "Paylasimli Cift", PaylasimliMiSnapshot = true, KapasiteSnapshot = 2 },
            new RezervasyonSegmentOdaAtama { Id = 8204, RezervasyonSegmentId = 8202, OdaId = 602, AyrilanKisiSayisi = 1, OdaNoSnapshot = "PAY-B", BinaAdiSnapshot = "Blok", OdaTipiAdiSnapshot = "Paylasimli Cift", PaylasimliMiSnapshot = true, KapasiteSnapshot = 2 });
        dbContext.RezervasyonKonaklayanlar.AddRange(
            new RezervasyonKonaklayan { Id = 8211, RezervasyonId = 8201, SiraNo = 1, AdSoyad = "Kadin Misafir", Cinsiyet = KonaklayanCinsiyetleri.Kadin, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi },
            new RezervasyonKonaklayan { Id = 8212, RezervasyonId = 8201, SiraNo = 2, AdSoyad = "Erkek Misafir", Cinsiyet = KonaklayanCinsiyetleri.Erkek, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi });
        dbContext.RezervasyonKonaklayanSegmentAtamalari.AddRange(
            new RezervasyonKonaklayanSegmentAtama { Id = 8221, RezervasyonKonaklayanId = 8211, RezervasyonSegmentId = 8202, OdaId = 601 },
            new RezervasyonKonaklayanSegmentAtama { Id = 8222, RezervasyonKonaklayanId = 8212, RezervasyonSegmentId = 8202, OdaId = 602 });

        // Uzatmanin IKINCI yarisinda (boundary->yeniCikis), oda 601 BASKA bir rezervasyondan ERKEK
        // konaklayanla, oda 602 ise BASKA bir rezervasyondan KADIN konaklayanla dolar - boylece
        // kadin ARTIK 601'de kalamaz (erkek-sabit), erkek ARTIK 602'de kalamaz (kadin-sabit); tek
        // gecerli eslesme TAKASTIR (kadin->602, erkek->601). Toplu oda-bazli karsilastirma
        // (segment1={601:1,602:1} -> segment2={601:1,602:1}) YAPISAL olarak AYNI gorunur (0
        // degisiklik gosterir) - ama GERCEKTE IKI konaklayan da yer degistirir.
        await SeedDigerPaylasimliRezervasyonuAsync(dbContext, 8231, 8232, odaId: 601, boundary, yeniCikis, KonaklayanCinsiyetleri.Erkek);
        await SeedDigerPaylasimliRezervasyonuAsync(dbContext, 8233, 8234, odaId: 602, boundary, yeniCikis, KonaklayanCinsiyetleri.Kadin);
        await dbContext.SaveChangesAsync();

        var tumKonaklayanlar = await dbContext.RezervasyonKonaklayanlar.Where(x => x.RezervasyonId == 8201).ToListAsync();
        var oncekiOda = new Dictionary<int, int> { [8211] = 601, [8212] = 602 };
        var oncekiYatak = new Dictionary<int, int?> { [8211] = null, [8212] = null };

        var segmentler = new List<KonaklamaSenaryoSegmentDto>
        {
            new() { BaslangicTarihi = cikis, BitisTarihi = boundary, OdaAtamalari = [UzatmaOdaAtamasi(601, 1, paylasimliMi: true), UzatmaOdaAtamasi(602, 1, paylasimliMi: true)] },
            new() { BaslangicTarihi = boundary, BitisTarihi = yeniCikis, OdaAtamalari = [UzatmaOdaAtamasi(601, 1, paylasimliMi: true), UzatmaOdaAtamasi(602, 1, paylasimliMi: true)] }
        };

        var sonuc = await InvokeComputeUzatmaCandidateAssignabilityAsync(
            dbContext, 8201, oncekiOda, oncekiYatak, segmentler, tumKonaklayanlar, requireNoMovesForFirstSegment: true);

        // Toplu hesap "degisiklik yok/tek degisiklik" gibi yaniltici bir sonuc verse bile, kisi
        // bazli dogrulama GERCEK takasi (2 hareket) tespit edip adayi REDDETMELIDIR.
        Assert.False(sonuc.GecerliMi);
        Assert.Equal(2, sonuc.ToplamGercekHareket);
    }

    [Fact]
    public async Task ComputeUzatmaCandidateAssignabilityAsync_GelmediKonaklayanHareketSayisinaVeYeniAtamalaraDahilEdilmez()
    {
        await using var dbContext = CreateDbContext();
        await SeedThreeRoomUzatmaFixtureAsync(dbContext);
        ...
        var cikis = new DateTime(2026, 3, 9, 10, 0, 0);
        var yeniCikis = new DateTime(2026, 3, 10, 10, 0, 0);
        var rezervasyonId = await SeedUzatmaRezervasyonuAsync(dbContext, 8301, 8302, odaId: 100, girisTarihi, cikis, kisiSayisi: 2);
        var aktifKonaklayanId = rezervasyonId * 100 + 1;

        // kisiSayisi=2 verildigi icin SeedUzatmaRezervasyonuAsync konaklayan kaydi OLUSTURMAZ (bkz.
        // yontem) - aktif VE Gelmedi konaklayanlari burada ELLE ekliyoruz.
        dbContext.RezervasyonKonaklayanlar.AddRange(
            new RezervasyonKonaklayan { Id = aktifKonaklayanId, RezervasyonId = rezervasyonId, SiraNo = 1, AdSoyad = "Aktif Misafir", Cinsiyet = null, KatilimDurumu = KonaklayanKatilimDurumlari.Geldi },
            new RezervasyonKonaklayan { Id = aktifKonaklayanId + 1, RezervasyonId = rezervasyonId, SiraNo = 2, AdSoyad = "Gelmedi Misafir", Cinsiyet = null, KatilimDurumu = KonaklayanKatilimDurumlari.Gelmedi });
        await dbContext.SaveChangesAsync();

        var tumKonaklayanlar = await dbContext.RezervasyonKonaklayanlar.Where(x => x.RezervasyonId == rezervasyonId).ToListAsync();

        // Gelmedi konaklayan hicbir zaman gercek bir oda atamasi almadigi icin oncekiOda haritasinda
        // YER ALMAZ - yalnizca aktif konaklayan icin bir onceki oda kaydedilir.
        var oncekiOda = new Dictionary<int, int> { [aktifKonaklayanId] = 100 };
        var oncekiYatak = new Dictionary<int, int?> { [aktifKonaklayanId] = null };

        var segmentler = new List<KonaklamaSenaryoSegmentDto>
        {
            new() { BaslangicTarihi = cikis, BitisTarihi = yeniCikis, OdaAtamalari = [UzatmaOdaAtamasi(101, 1)] }
        };

        var sonuc = await InvokeComputeUzatmaCandidateAssignabilityAsync(
            dbContext, rezervasyonId, oncekiOda, oncekiYatak, segmentler, tumKonaklayanlar, requireNoMovesForFirstSegment: false);

        // Yalnizca aktif konaklayanin GERCEK hareketi (100 -> 101) sayilir - Gelmedi konaklayan
        // hareket sayisina KATKI YAPMAZ ve donen segment planinda hic YER ALMAZ.
        Assert.True(sonuc.GecerliMi);
        Assert.Equal(1, sonuc.ToplamGercekHareket);
        var segmentPlani = Assert.Single(sonuc.SegmentPlanlari);
        var atama = Assert.Single(segmentPlani);
        Assert.Equal(aktifKonaklayanId, atama.KonaklayanId);
        Assert.DoesNotContain(segmentPlani, x => x.KonaklayanId == aktifKonaklayanId + 1);
    }

    private static string InvokeCreateUzatmaPlanKodu(
        int rezervasyonId,
        ...
        IReadOnlyList<KonaklamaSenaryoSegmentDto> segmentler)
    {
        var method = typeof(RezervasyonService).GetMethod("CreateUzatmaPlanKodu", BindingFlags.NonPublic | BindingFlags.Static)
            ?? throw new InvalidOperationException("CreateUzatmaPlanKodu metodu bulunamadi.");
        return (string)method.Invoke(null, [rezervasyonId, mevcutCikisTarihi, yeniCikisTarihi, segmentler])!;
    }

    private static async Task<List<(int KonaklayanId, int OdaId, int? YatakNo)>> InvokeComputeUzatmaGuestAssignmentPlanAsync(
        StysAppDbContext dbContext,
        int rezervasyonId,
        Dictionary<int, int> oncekiOda,
        Dictionary<int, int?> oncekiYatak,
        KonaklamaSenaryoSegmentDto plan,
        List<RezervasyonKonaklayan> tumKonaklayanlar,
        int kisiSayisi)
    {
        var service = CreateService(dbContext);
        var method = typeof(RezervasyonService).GetMethod("ComputeUzatmaGuestAssignmentPlanAsync", BindingFlags.NonPublic | BindingFlags.Instance)
        ...
        var task = (Task)method.Invoke(service, [
            rezervasyonId, oncekiOda, oncekiYatak, plan, tumKonaklayanlar, kisiSayisi, CancellationToken.None
        ])!;
        ...
        return plans
            .Select(x => (
                (int)x.GetType().GetProperty("KonaklayanId")!.GetValue(x)!,
                (int)x.GetType().GetProperty("OdaId")!.GetValue(x)!,
                (int?)x.GetType().GetProperty("YatakNo")!.GetValue(x)))
            .ToList();
    }

    private static KonaklamaSenaryoOdaAtamaDto UzatmaOdaAtamasi(int odaId, int ayrilanKisiSayisi, bool paylasimliMi = false, int kapasite = 2) =>
        new() { OdaId = odaId, AyrilanKisiSayisi = ayrilanKisiSayisi, PaylasimliMi = paylasimliMi, Kapasite = kapasite };

    private static async Task<(bool GecerliMi, int ToplamGercekHareket, List<List<(int KonaklayanId, int OdaId, int? YatakNo)>> SegmentPlanlari)> InvokeComputeUzatmaCandidateAssignabilityAsync(
        StysAppDbContext dbContext,
        int rezervasyonId,
        Dictionary<int, int> oncekiOda,
        Dictionary<int, int?> oncekiYatak,
    ...
    {
        var service = CreateService(dbContext);
        var method = typeof(RezervasyonService).GetMethod("ComputeUzatmaCandidateAssignabilityAsync", BindingFlags.NonPublic | BindingFlags.Instance)
            ?? throw new InvalidOperationException("ComputeUzatmaCandidateAssignabilityAsync metodu bulunamadi.");

        var task = (Task)method.Invoke(service, [
            rezervasyonId, oncekiOda, oncekiYatak, segmentler, tumKonaklayanlar, requireNoMovesForFirstSegment, CancellationToken.None
        ])!;

        await task;

        var resultProperty = task.GetType().GetProperty("Result") ?? throw new InvalidOperationException("Result ozelligi bulunamadi.");
        var sonuc = resultProperty.GetValue(task) ?? throw new InvalidOperationException("Sonuc null.");
        var sonucType = sonuc.GetType();

        var gecerliMi = (bool)sonucType.GetProperty("GecerliMi")!.GetValue(sonuc)!;
        var toplamGercekHareket = (int)sonucType.GetProperty("ToplamGercekHareket")!.GetValue(sonuc)!;
        var segmentPlanlariObj = (System.Collections.IEnumerable)sonucType.GetProperty("SegmentPlanlari")!.GetValue(sonuc)!;

        var segmentPlanlari = segmentPlanlariObj
            .Cast<object>()
            .Select(segmentPlan => ((System.Collections.IEnumerable)segmentPlan)
                .Cast<object>()
                .Select(x => (
                    (int)x.GetType().GetProperty("KonaklayanId")!.GetValue(x)!,
                    (int)x.GetType().GetProperty("OdaId")!.GetValue(x)!,
        ...
        return (gecerliMi, toplamGercekHareket, segmentPlanlari);
    }
