...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, FormsModule, ButtonModule, DialogModule, SelectModule, TagModule],
    template: `
        <!-- ── Context Bar ── -->
        <div class="muhasebe-tesis-context-bar flex align-items-center justify-content-between px-3 py-2 mb-3 border-round-lg surface-card border-1 border-surface-200 dark:border-surface-700 shadow-1">
            <div class="flex align-items-center gap-2">
                ...
                @if (context.seciliTesis(); as tesis) {
                    <p-tag [value]="tesis.ad" severity="info" styleClass="text-sm font-semibold px-3 py-1" />
                } @else {
                    <p-tag value="Tesisi seçilmedi" severity="warn" icon="pi pi-exclamation-triangle" styleClass="text-sm px-3 py-1" />
                }
            </div>

            <div class="flex align-items-center gap-2">
                @if (!context.seciliTesis()) {
                    <p-button label="Tesisi Seç" icon="pi pi-cog" size="small" severity="warn" (onClick)="degistirDialogAc()" />
                } @else {
                    <p-button label="Değiştir" icon="pi pi-pencil" size="small" severity="secondary" text (onClick)="degistirDialogAc()" />
                }
            </div>
        </div>

        <!-- ── Tesis Değiştirme Dialog'u ── -->
        <p-dialog [(visible)]="degistirDialogVisible" [modal]="true" [draggable]="false" header="Çalışma Tesisi Değiştir" [style]="{ width: '460px' }">
            <div class="flex flex-col gap-4">
                ...
                <div class="flex justify-end gap-2 pt-2">
                    @if (context.seciliTesis()) {
                        ...
                        <p-button label="İptal" icon="pi pi-times" severity="secondary" text (onClick)="degistirDialogVisible = false" />
                    }
                    <p-button label="Kaydet" icon="pi pi-check" [disabled]="!yeniTesisId() || context.tesislerLoading()" (onClick)="onDegistir()" />
                </div>
            </div>
        </p-dialog>
    `,
