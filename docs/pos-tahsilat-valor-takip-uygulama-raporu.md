...
commit 988b427) — güncelleme 6: 2026-07-23 (güncelleme 5'teki hatalı "AddDbContextFactory
context tipini kaydetmez" varsayımının düzeltilmesi, mükerrer AddScoped kaydının kaldırılması,
paylaşılan AddStysPersistence extension'ı, gerçek ICurrentUserAccessor/ICurrentTenantAccessor ile
DI testi, commit 988b427 sonrası)

## Güncelleme 6 — AddDbContextFactory'nin gerçek EF Core 10 davranışına göre düzeltilmesi (commit 988b427 sonrası)

`988b427`'deki DI düzeltmesi kısmen **hatalı bir varsayıma** dayanıyordu; bu turda 5 madde
düzeltildi:

1. **Kök neden (düzeltilen yanlış varsayım)**: Güncelleme 5'te "`AddDbContextFactory` yalnızca
   `IDbContextFactory<StysAppDbContext>`'i kaydeder, `StysAppDbContext`'in KENDİSİNİ kaydetmez"
   varsayımıyla ek bir `services.AddScoped(sp => factory.CreateDbContext())` satırı bırakılmıştı.
   Bu, **EF Core 10'un gerçek davranışına aykırıydı** — küçük bir prob testiyle (geçici olarak
   eklenip sonra kaldırıldı) doğrudan doğrulandı: `AddDbContextFactory<StysAppDbContext>(...,
   ServiceLifetime.Scoped)` tek başına `StysAppDbContext` için **tam olarak bir** Scoped
   descriptor kaydediyor. Ek `AddScoped` satırı bu yüzden `StysAppDbContext` için **ikinci,
   çakışan bir DI kaydı** oluşturuyordu — `GetRequiredService<StysAppDbContext>()` DI'ın "son
   kayıt kazanır" kuralı sayesinde çalışmaya devam ediyordu (bu yüzden önceki turun testleri
   yeşildi), ama `GetServices<StysAppDbContext>()`/`IEnumerable<StysAppDbContext>` çözümlemesi
   **iki** farklı context örneği döndürüyordu — potansiyel olarak kafa karıştırıcı/hataya açık bir
   durum.
2. **Düzeltme**: Fazladan `AddScoped(sp => factory.CreateDbContext())` satırı kaldırıldı.
   Doğrudan `StysAppDbContext` enjeksiyonunun `AddDbContextFactory`'nin kendi kaydıyla çalıştığı
   hem yeni bir birim testiyle (`StysAppDbContext_TekBirDescriptorOlarakKayitli_...`) hem de
   `PosTahsilatValorAktarimService` içeren tüm entegrasyon testlerinin (Senaryo 1-13) hâlâ yeşil
   olmasıyla doğrulandı.
3. **Paylaşılan `AddStysPersistence` extension'ı eklendi**
   (`backend/Infrastructure/EntityFramework/StysPersistenceServiceCollectionExtensions.cs`).
   `backend/Program.cs` VE `tests/STYS.Tests/DbContextFactoryDependencyInjectionTests.cs` artık
   **AYNI** metodu çağırıyor — DI testi, Program.cs'ten kopyalanmış/elle senkronize edilmesi
   gereken iki ayrı kayıt bloğu yerine, gerçekten production'da kullanılan kodu test ediyor.
4. **DI testi artık gerçek `ICurrentUserAccessor`/`ICurrentTenantAccessor` implementasyonlarını
   kullanıyor** (Fake değil): `TOD.Platform.Security.AddTodPlatformSecurity()` ile
   `HttpContextCurrentUserAccessor`/`HttpContextCurrentTenantAccessor` (production'da gerçekten
   kullanılan sınıflar) kaydediliyor; "iki farklı HTTP/request scope" her scope'un kendi
   `IHttpContextAccessor.HttpContext`'ine farklı claim'ler (`userId`, `kurumId` vb.) atanarak
   simüle ediliyor — `ICurrentUserAccessor`/`ICurrentTenantAccessor`'ın KENDİSİ sahte değil, yalnızca
   arkasındaki HTTP isteği simüle ediliyor. Test sırasında `IHttpContextAccessor`'ın
   (ASP.NET Core'da zorunlu olarak Singleton + AsyncLocal tabanlı) doğası nedeniyle iki scope'u
   ÖNCE kurup SONRA ikisini birden okumanın yanıltıcı bir test-artefaktı (ikinci scope'un
   AsyncLocal'ı ilkininkini de etkilemesi) ürettiği tespit edildi ve test, her "isteği" sırayla
   tam olarak tamamlayacak (kur → oku/yaz → kapat, sonra bir sonrakine geç) şekilde düzeltildi —
   bu, gerçek ASP.NET Core'un her isteği kendi izole `ExecutionContext`'inde çalıştırmasıyla
   tutarlıdır.
5. **`IServiceCollection` descriptor sayısı ve `IEnumerable<StysAppDbContext>` doğrudan test
   ediliyor**: yeni `StysAppDbContext_TekBirDescriptorOlarakKayitli_IEnumerableIkiContextUretmez`
   testi, `AddStysPersistence` sonrası `StysAppDbContext` için tam olarak **bir** descriptor
   (Scoped) olduğunu ve `GetServices<StysAppDbContext>()`'in tam olarak **bir** context ürettiğini
   doğrudan doğruluyor — gelecekte benzer bir "mükerrer kayıt" hatası tekrar eklenirse bu test
   kırılır.

Gerçek uygulama, Development ortamında `dotnet run` ile **ikinci kez** başlatılıp aynı doğrulamalar
tekrarlandı (migration'lar uygulandı, lisans doğrulandı, `/ui/version` ve `/health/live` 200
döndü, loglarda hiçbir DI/root-provider hatası yok) — düzeltmenin gerçek host'ta da hâlâ doğru
çalıştığı teyit edildi.

**Not (izole test container'ı hakkında şeffaflık)**: `DbContextFactoryDependencyInjectionTests`
tam bir `WebApplicationFactory<Program>` tabanlı host testi DEĞİLDİR — Program.cs'in yalnızca
persistence kaydını (artık `AddStysPersistence` ile paylaşılan gerçek kod) ve gerçek
`ICurrentUserAccessor`/`ICurrentTenantAccessor` implementasyonlarını içeren, odaklı bir
`ServiceCollection` kurar; Redis, JWT authentication, lisanslama, SignalR gibi raporlanan hatayla
İLGİSİZ alt sistemleri devreye almaz. Tam host'un da hatasız çalıştığı, ayrı ve tekrarlanabilir bir
adım olarak gerçek `dotnet run` ile doğrulanmıştır (aşağıdaki komutlara bakınız) — ancak bu, otomatik
bir test DEĞİL, manuel bir doğrulamadır.

### Değişen/eklenen dosyalar (bu güncellemede)
- `backend/Infrastructure/EntityFramework/StysPersistenceServiceCollectionExtensions.cs` (yeni,
  paylaşılan `AddStysPersistence` extension metodu)
- `backend/Program.cs` (iki satırlık kayıt bloğu `AddStysPersistence(connectionString)` çağrısıyla
  değiştirildi; yanlış "context tipini kaydetmez" yorumu düzeltildi)
- `tests/STYS.Tests/DbContextFactoryDependencyInjectionTests.cs` (gerçek `AddStysPersistence` +
  gerçek `AddTodPlatformSecurity` kullanacak şekilde yeniden yazıldı; yeni descriptor-sayısı testi
  eklendi; iki-scope testi AsyncLocal-güvenli sıralı yapıya kavuşturuldu)

### Çalıştırılan komutlar ve sonuçlar (yeniden üretilebilir)
- **Prob testi** (geçici, EF Core 10'un gerçek davranışını doğrulamak için eklenip kaldırıldı):
  `AddDbContextFactory<StysAppDbContext>(options, ServiceLifetime.Scoped)` sonrası
  `services.Where(d => d.ServiceType == typeof(StysAppDbContext))` → **1 descriptor, Scoped**
  (varsayımın hatalı olduğu doğrudan kanıtlandı).
...
- `dotnet test --filter FullyQualifiedName~DbContextFactoryDependencyInjectionTests` → **3/3
  Passed** (descriptor-sayısı testi + tek-scope çözümleme testi + iki-scope sızıntı testi, gerçek
  `ICurrentUserAccessor`/`ICurrentTenantAccessor` ile).
- `dotnet test --filter FullyQualifiedName~PosTahsilatValorIntegrationTests` (gerçek SQL Server'a
  karşı, 13 senaryo) → **13/13 Passed, 0 Failed, 0 Skipped** (ilk denemede ortamsal bir
  yavaşlama/geçici bağlantı sorunu nedeniyle 2 test başarısız oldu — SQL Server'da hiçbir blocking
  session olmadığı doğrulandıktan sonra paket TEKRAR çalıştırıldı ve 13/13 temiz geçti; bu, kod
  regresyonu değil, ortam kaynaklı geçici bir durumdu).
- `dotnet test tests/STYS.Tests` (tüm proje, env var ayarlı) → **331 Passed, 0 Failed, 0
  Skipped**.
- Gerçek uygulama `dotnet run` ile Development ortamında **ikinci kez** başlatıldı (dev SQL
  Server'a karşı, `ASPNETCORE_ENVIRONMENT=Development dotnet run --no-launch-profile --urls
  http://localhost:5299`) → migration'lar uygulandı, lisans doğrulandı, `curl http://localhost:5299/ui/version`
  ve `curl http://localhost:5299/health/live` ikisi de 200 döndü, `PosValorAktarimHostedService`'in
  gerçek scoped `StysAppDbContext` çözümlemesi hatasız çalıştı (loglarda DI/root-provider hatası
  yok) → uygulama durduruldu.
...
3. ~~**Mükerrer kayıt kontrolü**: `AddDbContextFactory`, `StysAppDbContext`'in KENDİSİNİ DI'a ayrıca
   kaydetmez...~~ **DÜZELTME (bkz. Güncelleme 6): bu madde HATALIYDI.** EF Core 10'da
   `AddDbContextFactory`, `StysAppDbContext`'i DE (Scoped) kaydeder; aşağıdaki `AddScoped(sp =>
   factory.CreateDbContext())` satırı gerçekte GEREKSİZ ve `StysAppDbContext` için ikinci, çakışan
   bir DI kaydı oluşturuyordu. Güncelleme 6'da kaldırıldı.
