...
[Migration("20260707090000_AddOdaMusaitlikRaporuPermissionsAndMenu")]
public partial class AddOdaMusaitlikRaporuPermissionsAndMenu : Migration
{
    protected override void Up(MigrationBuilder migrationBuilder)
    {
        migrationBuilder.Sql(
            ...
            -- Permission rolleri: OdaMusaitlikRaporuYonetimi.Menu ve .View
            ...
            INSERT INTO @RequiredRoles ([Domain], [Name]) VALUES
                (N'OdaMusaitlikRaporuYonetimi', N'Menu'),
                (N'OdaMusaitlikRaporuYonetimi', N'View');
            ...
            DECLARE @MenuRoleId uniqueidentifier = (
                ...
                WHERE [Domain] = N'OdaMusaitlikRaporuYonetimi' AND [Name] = N'Menu'
            );
            DECLARE @ViewRoleId uniqueidentifier = (
                SELECT TOP (1) [Id] FROM [TODBase].[Roles]
                WHERE [Domain] = N'OdaMusaitlikRaporuYonetimi' AND [Name] = N'View'
            );
            ...
            -- Grup menü item'a OdaMusaitlikRaporuYonetimi.Menu rolünü bağla (görünürlük için)
            IF NOT EXISTS (
            ...
            -- Bos Oda / Musaitlik Raporu menu item
            DECLARE @OdaMusaitlikMenuItemId uniqueidentifier;
            SELECT TOP (1) @OdaMusaitlikMenuItemId = [Id]
            FROM [TODBase].[MenuItems]
            WHERE [Route] = N'raporlar/oda-musaitlik' AND [IsDeleted] = 0;

            IF @OdaMusaitlikMenuItemId IS NULL
            BEGIN
                SET @OdaMusaitlikMenuItemId = NEWID();
                INSERT INTO [TODBase].[MenuItems] ([Id], [Label], [Icon], [Route], [QueryParams], [ParentId], [MenuOrder], [IsDeleted], [CreatedAt], [UpdatedAt])
                VALUES (@OdaMusaitlikMenuItemId, N'Bos Oda / Musaitlik Raporu', N'pi pi-home', N'raporlar/oda-musaitlik', NULL, @RaporlarMenuGrupId, 5, 0, @Now, @Now);
            END
            ELSE
            BEGIN
                UPDATE [TODBase].[MenuItems]
                SET [Label] = N'Bos Oda / Musaitlik Raporu',
                    [Icon] = N'pi pi-home',
                    ...
                    [DeletedAt] = NULL,
                    [UpdatedAt] = @Now
                WHERE [Id] = @OdaMusaitlikMenuItemId;
            END;

            -- Menü görünürlük rolü: OdaMusaitlikRaporuYonetimi.Menu
            IF NOT EXISTS (
                SELECT 1 FROM [TODBase].[MenuItemRoles]
                WHERE [MenuItemId] = @OdaMusaitlikMenuItemId AND [RoleId] = @MenuRoleId AND [IsDeleted] = 0
            )
                INSERT INTO [TODBase].[MenuItemRoles] ([Id], [MenuItemId], [RoleId], [IsDeleted], [CreatedAt], [UpdatedAt])
                VALUES (NEWID(), @OdaMusaitlikMenuItemId, @MenuRoleId, 0, @Now, @Now);
            """);
    }
    ...
    protected override void Down(MigrationBuilder migrationBuilder)
    {
        migrationBuilder.Sql(
            ...
            SET [IsDeleted] = 1, [UpdatedAt] = SYSUTCDATETIME()
            WHERE [Domain] = N'OdaMusaitlikRaporuYonetimi' AND [IsDeleted] = 0;
            """);
    }
}
