...
public class StysAppDbContext : DbContext
{
    ...
    public DbSet<TesisMuhasebeci> TesisMuhasebecileri => Set<TesisMuhasebeci>();
    ...
    public DbSet<PaketTuru> PaketTurleri => Set<PaketTuru>();
    ...
    public DbSet<KantinSatisIade> KantinSatisIadeleri => Set<KantinSatisIade>();
    public DbSet<KantinSatisIadeSatir> KantinSatisIadeSatirlari => Set<KantinSatisIadeSatir>();
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        {
            entity.ToTable("TesisMuhasebecileri", "dbo");
            entity.HasIndex(x => new { x.TesisId, x.UserId })
                .IsUnique()
                .HasFilter("[IsDeleted] = 0");

            entity.HasOne(x => x.Tesis)
                .WithMany(x => x.Muhasebeciler)
                .HasForeignKey(x => x.TesisId)
                .OnDelete(DeleteBehavior.Restrict);
        });
        ...
        modelBuilder.Entity<MuhasebeHesapKoduSayac>(entity =>
        {
            ...
            entity.Property(x => x.AnaHesapKodu).HasMaxLength(50).IsRequired();
            entity.Property(x => x.Aciklama).HasMaxLength(512);
            entity.Property(x => x.RowVersion).IsRowVersion().IsConcurrencyToken();
            entity.HasIndex(x => new { x.TesisId, x.AnaHesapKodu })
                .IsUnique()
                .HasFilter("[IsDeleted] = 0");
            entity.HasIndex(x => x.TesisId)
                .HasFilter("[IsDeleted] = 0");
        ...
        modelBuilder.Entity<CariHareket>(entity =>
        {
            ...
            entity.Property(x => x.KapananTutar).HasPrecision(18, 2).HasDefaultValue(0m);
            entity.Property(x => x.KalanTutar).HasPrecision(18, 2).HasDefaultValue(0m);
            entity.Property(x => x.ParaBirimi).HasMaxLength(3).IsRequired();
            ...
            entity.HasIndex(x => new { x.CariKartId, x.KapandiMi })
                .HasFilter("[IsDeleted] = 0");
            entity.HasIndex(x => x.IliskiliCariHareketId)
                .HasFilter("[IsDeleted] = 0 AND [IliskiliCariHareketId] IS NOT NULL");
            ...
            entity.HasOne(x => x.IliskiliCariHareket)
                .WithMany()
                .HasForeignKey(x => x.IliskiliCariHareketId)
                .OnDelete(DeleteBehavior.Restrict);
        });
        ...
        modelBuilder.Entity<PaketTuru>(entity =>
        {
            entity.ToTable("PaketTurleri", muhasebeSchema);
            entity.Property(x => x.Ad).HasMaxLength(128).IsRequired();
            entity.Property(x => x.KisaAd).HasMaxLength(16).IsRequired();
            entity.HasIndex(x => x.Ad)
                .IsUnique()
                .HasFilter("[IsDeleted] = 0");
            entity.HasIndex(x => x.KisaAd)
                .IsUnique()
                .HasFilter("[IsDeleted] = 0");
            entity.HasIndex(x => x.AktifMi)
                .HasFilter("[IsDeleted] = 0");
        ...
        modelBuilder.Entity<KantinSatisIade>(entity =>
        {
            entity.ToTable("KantinSatisIadeleri", kantinSchema);
            entity.Property(x => x.Durum).HasMaxLength(16).IsRequired();
            entity.Property(x => x.Aciklama).HasMaxLength(1024);
            entity.Property(x => x.OlusturanKullaniciId).HasMaxLength(64);
            entity.Property(x => x.FinansalIadeDurumu).HasMaxLength(16).IsRequired();
        ...
        modelBuilder.Entity<KantinSatisIadeSatir>(entity =>
        {
            entity.ToTable("KantinSatisIadeSatirlari", kantinSchema);
            entity.Property(x => x.Miktar).HasPrecision(18, 3);
            entity.Property(x => x.StokKodu).HasMaxLength(64).IsRequired();
            entity.Property(x => x.UrunAdi).HasMaxLength(256).IsRequired();
            entity.Property(x => x.Birim).HasMaxLength(32).IsRequired();
            entity.Property(x => x.TakipTipi).HasMaxLength(16);
            entity.Property(x => x.LotNo).HasMaxLength(128);
            entity.Property(x => x.SeriNo).HasMaxLength(128);
            entity.Property(x => x.BirimSatisFiyati).HasPrecision(18, 2);
            entity.Property(x => x.KdvOrani).HasPrecision(5, 2);
            ...
            entity.HasIndex(x => new { x.KantinSatisIadeId, x.KantinSatisSatirId })
                .IsUnique()
                .HasFilter("[IsDeleted] = 0");

            entity.HasIndex(x => x.KantinSatisSatirId)
                .HasFilter("[IsDeleted] = 0");
            ...
            entity.HasOne(x => x.KantinSatisIade)
                .WithMany(x => x.Satirlar)
                .HasForeignKey(x => x.KantinSatisIadeId)
                .OnDelete(DeleteBehavior.Cascade);

            entity.HasOne(x => x.KantinSatisSatir)
                .WithMany()
                .HasForeignKey(x => x.KantinSatisSatirId)
                .OnDelete(DeleteBehavior.Restrict);
        ...
        modelBuilder.Entity<MuhasebeVergiHesapEsleme>(entity =>
        {
            entity.ToTable("MuhasebeVergiHesapEslemeleri", muhasebeSchema);
            entity.Property(x => x.VergiTipi).HasMaxLength(32).IsRequired();
            entity.Property(x => x.Oran).HasPrecision(5, 2);
            entity.Property(x => x.AktifMi).IsRequired();
            entity.HasIndex(x => x.AlisKdvHesapId);
            entity.HasIndex(x => x.SatisKdvHesapId);
            entity.HasIndex(x => new { x.VergiTipi, x.Oran })
                .IsUnique()
                .HasFilter("[IsDeleted] = 0 AND [AktifMi] = 1 AND [TesisId] IS NULL");
            entity.HasIndex(x => new { x.TesisId, x.VergiTipi, x.Oran })
                .IsUnique()
                .HasFilter("[IsDeleted] = 0 AND [AktifMi] = 1 AND [TesisId] IS NOT NULL");

            entity.HasOne(x => x.AlisKdvHesap)
                .WithMany()
                .HasForeignKey(x => x.AlisKdvHesapId)
                .OnDelete(DeleteBehavior.Restrict);
        ...
        {
            entity.ToTable("StokSayimlar", muhasebeSchema);
            entity.Property(x => x.Durum).HasMaxLength(16).IsRequired();
            entity.Property(x => x.Aciklama).HasMaxLength(1024);
            entity.HasIndex(x => new { x.TesisId, x.DepoId, x.SayimTarihi })
                .HasFilter("[IsDeleted] = 0");
        ...
        modelBuilder.Entity<StokSayimSatir>(entity =>
        {
            entity.ToTable("StokSayimSatirlari", muhasebeSchema);
            entity.Property(x => x.TakipTipi).HasMaxLength(16).IsRequired();
            entity.Property(x => x.StokKodu).HasMaxLength(64).IsRequired();
            ...
            entity.Property(x => x.SeriNo).HasMaxLength(128);
            entity.Property(x => x.SistemMiktari).HasPrecision(18, 3);
            entity.Property(x => x.SayilanMiktar).HasPrecision(18, 3);
            entity.Property(x => x.FarkMiktari).HasPrecision(18, 3);
            entity.HasIndex(x => new { x.StokSayimId, x.TasinirKartId, x.StokLotId, x.StokSeriId })
                .HasFilter("[IsDeleted] = 0");
        ...
        modelBuilder.Entity<StokSeri>(entity =>
        {
            entity.ToTable("StokSeriler", muhasebeSchema);
            entity.Property(x => x.SeriNo).HasMaxLength(128).IsRequired();
            entity.Property(x => x.Aciklama).HasMaxLength(1024);
            entity.HasIndex(x => new { x.TesisId, x.TasinirKartId, x.SeriNo })
                .IsUnique()
                .HasFilter("[IsDeleted] = 0");
        ...
        modelBuilder.Entity<SatisBelgesi>(entity =>
        {
            entity.ToTable("SatisBelgeleri", muhasebeSchema, t =>
            {
                ...
                // AYRIŞTIRILMIŞ VE ARTIK OTORİTER olan üç durum alanı (bkz. SatisBelgesiDurumProjection)
                // - NON-NULLABLE'dır (MakeSeparatedSatisBelgesiStatusesAuthoritative migration'ı ile
                // NULL bırakan/bırakabilecek satırlar önce backfill edilip sonra kolonlar NOT NULL
                // yapılmıştır) ve yalnızca ilgili enum'un tanımlı değerlerini taşıyabilirler.
                // TicariBelgeDurumu: Taslak=1, Hazir=2, IptalEdildi=3.
                t.HasCheckConstraint(
                    "CK_SatisBelgeleri_TicariDurum",
                    "[TicariDurum] IN (1, 2, 3)");

                // TicariBelgeMuhasebeDurumu: Bekliyor=1, Onayda=2, Onaylandi=3, Reddedildi=4, IptalEdildi=5.
                t.HasCheckConstraint(
                    "CK_SatisBelgeleri_MuhasebeDurumu",
                    "[MuhasebeDurumu] IN (1, 2, 3, 4, 5)");
                ...
                t.HasCheckConstraint(
                    ...
                    "[FaturalamaDurumu] IN (1, 2, 3, 4, 5, 6)");
            });
